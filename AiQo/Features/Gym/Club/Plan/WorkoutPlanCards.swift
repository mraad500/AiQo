...
struct ActivePlanCard: View {
    ...
    let onStartWorkout: (WorkoutPlan) -> Void
    let onRefresh: () -> Void
    let onShare: () -> Void

    @State private var selectedDayIndex: Int = 0

    private var insights: WorkoutPlanInsights {
        planForCurrentSelection.insights(language: language)
    }
    private var isArabic: Bool { language == .arabic }

    private var hasDays: Bool { (plan.days?.isEmpty == false) }
    private var safeSelectedDayIndex: Int {
        guard let days = plan.days, !days.isEmpty else { return 0 }
        return min(max(selectedDayIndex, 0), days.count - 1)
    }
    private var displayedExercises: [(globalIndex: Int, exercise: Exercise)] {
        guard let days = plan.days, !days.isEmpty else {
            return plan.exercises.enumerated().map { ($0.offset, $0.element) }
        }
        let day = days[safeSelectedDayIndex]
        let offset = days.prefix(safeSelectedDayIndex).map(\.exercises.count).reduce(0, +)
        return day.exercises.enumerated().map { (offset + $0.offset, $0.element) }
    }

    private var completedCount: Int {
        displayedExercises.filter { completionByIndex[$0.globalIndex] == true }.count
    }
    private var progress: Double {
        guard !displayedExercises.isEmpty else { return 0 }
        return Double(completedCount) / Double(displayedExercises.count)
    }
    private var allDone: Bool { progress >= 1.0 && !displayedExercises.isEmpty }
                ...
                ForEach(Array((plan.days ?? []).enumerated()), id: \.offset) { index, day in
                    Button {
                        ...
                        Text(day.name)
                            ...
                            .lineLimit(1)
                            .foregroundStyle(safeSelectedDayIndex == index ? PlanPalette.mintDeep : PlanPalette.textPrimary.opacity(0.7))
                            .padding(.horizontal, 12)
                            .padding(.vertical, 7)
                            .background(
                                Capsule(style: .continuous)
                                    .fill(safeSelectedDayIndex == index ? PlanPalette.mint : Color.clear)
                            )
                            .overlay(
                                Capsule(style: .continuous)
                                    .stroke(safeSelectedDayIndex == index ? Color.clear : PlanPalette.hairline, lineWidth: 1)
                            )
                    }
                    .buttonStyle(.plain)
                }
    ...
    private var metaRow: some View {
        HStack(spacing: 18) {
            ...
            metaDivider
            metaCell(
                value: "\(displayedExercises.count)",
                unit: nil,
                label: isArabic ? "تمارين" : "moves"
            )
            metaDivider
            metaCell(
                value: "\(displayedSetsTotal)",
                unit: nil,
    ...
    private var displayedSetsTotal: Int {
        displayedExercises.reduce(0) { $0 + $1.exercise.sets }
    }
    ...
    private var exerciseList: some View {
        VStack(spacing: 0) {
            ForEach(Array(displayedExercises.enumerated()), id: \.offset) { rowIndex, entry in
                ActivePlanExerciseRow(
                    index: rowIndex,
                    exercise: entry.exercise,
                    isCompleted: completionByIndex[entry.globalIndex] ?? false,
                    language: language,
                    onTap: { onTapExercise(entry.exercise) },
                    onToggle: { onToggleCompletion(entry.globalIndex) }
                )
                if rowIndex < displayedExercises.count - 1 {
                    Rectangle()
                        .fill(PlanPalette.hairline)
                        .frame(height: 1)
                        .padding(.leading, 50)
    ...
    private var startButton: some View {
        Button {
            onStartWorkout(planForCurrentSelection)
        } label: {
            HStack(spacing: 10) {
    ...
    private var planForCurrentSelection: WorkoutPlan {
        guard let days = plan.days, !days.isEmpty else { return plan }
        let day = days[safeSelectedDayIndex]
        let dayTitle = day.name.isEmpty ? plan.title : "\(plan.title) — \(day.name)"
        return WorkoutPlan(
            title: dayTitle,
