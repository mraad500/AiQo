...
final class QuestDailyStore: ObservableObject {
    ...
    private var stage2DailyChallenges: [Challenge] {
        challenges.filter { $0.stageNumber == 2 && !$0.isBoss }
    }

    private var stage2BossChallenge: Challenge? {
        challenges.first { $0.stageNumber == 2 && $0.isBoss }
    }
    ...
    func progressText(for challenge: Challenge) -> String {
        ...
        case .questCompletions:
            return "\(Int(current.rounded()))/\(stage2DailyChallenges.count) \(L10n.t("quests.unit.quests"))"
        case .steps:
    ...
    func addPushups(_ reps: Int, for challenge: Challenge) {
        ...
        guard challenge.metricType == .pushups else { return }
        ...
        pushupUndoStacks[challenge.id, default: []].append(reps)
    ...
    func canUndoPushups(for challenge: Challenge) -> Bool {
        guard challenge.metricType == .pushups else { return false }
    ...
    private func refreshBossProgress() {
        guard let boss = stage2BossChallenge else { return }

        let completedCount = stage2DailyChallenges.reduce(0) { partial, challenge in
    ...
    private func proofValue(for challenge: Challenge, achievedValue: Double) -> String {
        ...
        case .questCompletions:
            return "\(L10n.t("quests.metric.stage2")): \(Int(achievedValue.rounded()))/\(stage2DailyChallenges.count) \(L10n.t("quests.unit.quests"))"
        }
    }
