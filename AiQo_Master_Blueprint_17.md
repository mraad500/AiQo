...
## 35. Captain Brain Refactor — 14-Layer Reasoning Architecture & 6-Round Cross-Layer Audit (2026-05-09 → 2026-05-10)

The headline pass on this branch (`brain-refactor/p-fix-dev-override`, three commits: `d955c04` brain, `83ef407` keyboard fix, `85de575` kitchen WIP). Triggered by a screenshot bug captured 2026-05-09: the Captain told the user `"خل نمشي مشية خفيفة بزون 2"` immediately after `"آخر تمرين الك جان مشي، استمريت بيه ٤٥ دقيقة وحرقت ٢٦١ سعرة"` — i.e. recommended walking to a user who'd just told it they walked 45 minutes. Root cause was structural, not prompt-quality: the prompt had access to *recent steps* but no structured *recent activity* fact, so the model had to infer "user just walked" from chat history alone and silently failed to honor that inference when generating the next suggestion.

The fix is a **14-layer reasoning brain** that synthesizes a deterministic `ReasoningBrief` *before* the prompt is built, plus a **post-generation `ResponseQualityGate`** that audits the model's reply against the brief and (behind a feature flag) triggers a single corrective regen on critical failures. **Six audit rounds (§50 in code comments)** then tightened cross-layer communication so no two layers ship contradictory directives.

Thirteen new Swift files in `Brain/01_Sensing` + `Brain/03_Reasoning` + `Brain/04_Inference`, eight modified, ten new test files (≈64 unit tests), plus one one-line UX fix in `CaptainChatView` for a keyboard-occlusion bug the user surfaced mid-pass. Net delta: **+~3,500 LOC of brain code + tests**, zero pbxproj edits (Xcode 16 sync), `BUILD SUCCEEDED` with zero errors / zero warnings on the iPhone 17 Pro simulator.

### 35.1 The bug-of-record

```
User:     "آخر تمرين الك جان مشي، استمريت بيه ٤٥ دقيقة..."
Captain:  "أدائك جان كلش زين، 45 دقيقة مشي ثبات ممتاز..."
User:     "شنو انسوي اب كيفك ؟؟"
Captain:  "محمد، بما إنك تحب المشي، شرايك اليوم نمشي مشية خفيفة بزون 2؟ ⚠️"
User:     "نته شبيك ؟؟؟ هسوني مشيت 45 دقيقة شبيكككك 😡😒"
```

The Captain *knew* about the walk (it correctly summarized it one turn earlier) but didn't carry that fact into the *suggestion-generation* path. Turn N referenced the workout; turn N+2 forgot it existed. This is the failure mode the entire §35 refactor is engineered around — and the §41 quality gate's `usedAvoidedFamily` check is the empirical receipt that proves it doesn't recur.

### 35.2 Architecture

The brain has been a 7-layer prompt composer since the original v1.0 cut. §35 doesn't replace those layers — it adds a *synthesis stage* upstream of the composer, plus a *gate stage* downstream of Gemini:

```
USER MESSAGE
   ↓
[CAPTURE]   §33 RecentActivitySnapshot (sync, UserDefaults)
            §34 ConversationCoherenceAnalyzer (sync, scans last 5 user turns)
            §46 EpisodicRecallProvider (async, reads EpisodicStore actor)
            §49 HRMoodReader (async, two parallel HealthKit reads)
   ↓
[PROFILE]   §39 UserProfileLens (sync, age + experience + goal)
            §38 HabitDetector (sync, scans 7-day buffer + last 7 workouts)
            §42 PersonalityAnalyzer (sync, scans last 8 user turns)
            §44 BehavioralStageDetector (sync, TTM stage from streak + daily points)
   ↓
[REASONING] §40 MicroInsightGenerator (sync, today-specific observations)
            §43 CausalChainBuilder (sync, activity → state → intention)
            §45 PredictiveIntentEngine (sync, anticipated follow-up)
            §47 WorkoutSpecialistReasoner (sync, intent-gated load reasoning)
            §37 CognitiveReasoner.reason(input:) — synthesizes everything into ReasoningBrief
   ↓
[INVARIANTS]§50 brief.enforceInvariantsInDebug() — 7 cross-layer contradiction checks
   ↓
[PROMPT]    PromptComposer.build() renders 15 layers; the brief becomes layerReasoningBrief
   ↓
GEMINI API
   ↓
[GATE]      §41 ResponseQualityGate.evaluate(reply:request:) — 8 deterministic checks
            §48 IraqiDialectScorer (Iraqi-vs-MSA token ratio)
            On critical violation + CAPTAIN_QUALITY_REGEN_ENABLED: re-issue once with
            a corrective prefix that carries the brief's thesis as the upstream cause
   ↓
REPLY TO USER
```

Every box on the left is a *new file*. Every box is also *deterministic* — no LLM calls inside the brain, only inside the prompt → cloud step. Total brain compute on a typical message is **<5 ms on MainActor**; the bottleneck is the parallel HealthKit + SwiftData fetches in the capture stage, bounded by the existing 2-second-per-query timeout.

### 35.3 Layer breakdown — §33 through §49

Each layer is a separate file under `AiQo/Features/Captain/Brain/` plus a corresponding test file under `AiQoTests/`. The internal `§NN` numbers are doc-comment markers; the surface area is the sum of these layers' contributions to `ReasoningBrief`.

| § | Layer | Purpose | Output field on `ReasoningBrief` |
|---|---|---|---|
| 33 | [`RecentActivitySnapshot`](AiQo/Features/Captain/Brain/03_Reasoning/RecentActivitySnapshot.swift) | Most recent tracked workout (≤24h) as a structured fact: family, duration, calories, distance, freshness band (`veryFresh` / `fresh` / `stale`). Anti-repeat anchor. | `(consumed by other layers; rendered in §10 of PromptComposer)` |
| 34 | [`ConversationCoherenceAnalyzer`](AiQo/Features/Captain/Brain/03_Reasoning/ConversationCoherenceAnalyzer.swift) | Scans last 5 user turns for: completed-activity claims (`"مشيت 45د"`), refusals (`"ما أبي ركض"`), latest emotion, frustration with the Captain itself. Splits avoidances into `hard` (refusals) vs `soft` (completions). | `coherenceTags` on `CaptainContextData` (rendered separately in §11) |
| 35 | `ConversationThread.buildPromptSummary` | Replaces the v1 50-char-truncation log with two-block output: structured tags (`workoutDetected`, `goalCompleted`) + a longer chronological timeline (110-char cap). | `recentInteractions` on `CaptainContextData` |
| 36 | Identity layer `REPLY QUALITY` block | Added to both Arabic and English persona blocks: A. connect-to-recent-context, B. propose-don't-ask-blind, C. numbers-when-they-help, D. no-contradiction, E. one-suggestion-per-reply, F. consult-the-reasoning-brief. | (prompt directive; not a brief field) |
| 37 | [`CognitiveReasoner`](AiQo/Features/Captain/Brain/03_Reasoning/CognitiveReasoner.swift) | The executive function. `reason(input:)` consumes everything below and emits `ReasoningBrief` with 17 fields: thesis + angle + observedPatterns + smartCallbacks + openingHook + nextDayHint + avoidances + habitPatterns + profileDirective + microInsights + styleDirective + causalChain + behavioralStage + predictedFollowUp + episodicRecall + specialistGuidance + hrMood. | the brief itself |
| 38 | [`HabitDetector`](AiQo/Features/Captain/Brain/03_Reasoning/HabitDetector.swift) | 6 rhythm patterns (training-days-of-week, morning/evening exerciser, family preference, sleep consistency, no-rest streak, weekend warrior). Confidence-floored at 0.6, capped at 3 patterns total — and only **1** pattern for users in `relapse`/`contemplation` stages so the brief stays warm rather than diagnostic. | `habitPatterns` |
| 39 | [`UserProfileLens`](AiQo/Features/Captain/Brain/03_Reasoning/UserProfileLens.swift) | Demographic-aware coaching directive. Maps age → bracket (`youth`/`prime`/`established`/`senior`), level + workout-count → experience (`novice`/`intermediate`/`advanced`), and primary goal → coaching anchor. Calibrates downstream thresholds (steps personal-best floor, low-sleep threshold, HR fallback baseline). | `profileDirective` |
| 40 | [`MicroInsightGenerator`](AiQo/Features/Captain/Brain/03_Reasoning/MicroInsightGenerator.swift) | 7 today-specific observations (first-active-after-rest, streak milestone every 7 days, best/worst sleep in window, longest workout in window, first-of-period family, steps-vs-yesterday-pace). Capped at 2 insights to keep the brief compact. | `microInsights` |
| 41 | [`ResponseQualityGate`](AiQo/Features/Captain/Brain/04_Inference/ResponseQualityGate.swift) | Post-generation audit. 8 violation kinds: `usedAvoidedFamily(severity)`, `missedFreshActivityReference`, `vagueClose`, `noSpecificNumber`, `excessiveLength`, `dialectDrift` (English in Arabic mode), `msaDriftInArabicMode` (§48), `angleViolation(expected)` (§50). Critical violations drop the score below the 0.6 acceptance floor; behind `CAPTAIN_QUALITY_REGEN_ENABLED` a single corrective regen fires. | (post-gen, no brief field) |
| 42 | [`PersonalityStyleVector`](AiQo/Features/Captain/Brain/03_Reasoning/PersonalityStyleVector.swift) | 6-dimensional style fingerprint from last 8 user turns: avg-words, emoji-frequency, punctuation-intensity, formality (MSA-tells), question-density, preferred-opening (greeting/direct/question). Renders into a one-paragraph "voice mirror" directive. Suppressed below 2-message sample. | `styleDirective` |
| 43 | [`CausalKnowledgeGraph`](AiQo/Features/Captain/Brain/03_Reasoning/CausalKnowledgeGraph.swift) | Static directed graph: 13 activity nodes → 6 body-state nodes → 9 intention nodes, ~50 weighted edges, each grounded in standard ACSM/NSCA literature. `CausalChainBuilder.derive(...)` walks the graph for 2- or 3-hop chains. **Stage-aware depth**: relapse/contemplation get 1-hop only. **Specialist-aware skip**: when the workout specialist will fire, the chain is suppressed entirely so the model gets *one* canonical explanation, not two. | `causalChain` |
| 44 | [`BehavioralStageEngine`](AiQo/Features/Captain/Brain/03_Reasoning/BehavioralStageEngine.swift) | Transtheoretical-Model stage detector (Prochaska & DiClemente). Cascading rules: relapse (10+ day inactive gap with prior activity) > maintenance (streak ≥30 + 5+ workouts in 14d) > action (streak ≥5 + 3+ workouts in 7d) > preparation (streak ≥1 OR commitment language) > contemplation (active session, no streak). Stage drives the coaching playbook directive *and* feeds back into the angle picker (level 4) and habit cap. | `behavioralStage` |
| 45 | [`PredictiveIntentEngine`](AiQo/Features/Captain/Brain/03_Reasoning/PredictiveIntentEngine.swift) | Anticipates the user's likely next question via 6-priority cascade. **HR-aware** at the top: stressed body → predict `sleepConcern`; postEffort → `nutritionAfterEffort`; windingDown → `sleepConcern`. Then fresh-activity rules, late+tired rules, low-sleep-morning rules, challenge-intent rules. Confidence-floored at 0.65; the brief renderer suppresses below the floor. | `predictedFollowUp` |
| 46 | [`EpisodicRecallProvider`](AiQo/Features/Captain/Brain/03_Reasoning/EpisodicRecallProvider.swift) | Async actor query against the existing `EpisodicStore` for the top-3 high-salience entries within 14 days. Renders compact "from earlier sessions" lines (Arabic + English) that the prompt presents to the model as optional callback material. The first cross-session memory channel — the Captain stops being stateless. | `episodicRecall` |
| 47 | [`SpecialistReasoners`](AiQo/Features/Captain/Brain/03_Reasoning/SpecialistReasoners.swift) | Domain-specialist sub-reasoner. Phase-12 ships `WorkoutSpecialistReasoner` only; `SpecialistGuidance` is a sum type with reserved cases for nutrition + sleep specialists. The workout specialist computes `LoadSignals` (consecutive-training-days, high-intensity-yesterday, elevated-RHR, low-sleep, very-fresh-activity), picks an intensity, then **clamps to the brief's angle** (repair/grounding ≤ light, gentle ≤ moderate, recovery ≤ light) so the specialist can never contradict the angle picker. | `specialistGuidance` |
| 48 | [`IraqiDialectScorer`](AiQo/Features/Captain/Brain/04_Inference/IraqiDialectScorer.swift) | Counts Iraqi-dialect tokens vs MSA tells in the reply. Surfaced as a critical Quality Gate violation (`msaDriftInArabicMode`) when the Iraqi share drops below 0.4 with at least 3 dialect-bearing tokens present. The §41 dialect-drift check only catches *English* in Arabic mode; §48 catches Arabic-but-formal — i.e. the Captain slipped into Modern Standard Arabic instead of the persona's Iraqi register. | (post-gen) |
| 49 | [`HRMoodReader`](AiQo/Features/Captain/Brain/01_Sensing/HRMoodReader.swift) | Real-time mood inference from live HR vs resting HR vs recent activity. Live HR is the latest sample within the last 5 minutes (HealthKit query, 2s timeout). Resting HR is the latest `restingHeartRate` sample (HealthKit derives this overnight on Apple Watch). Two parallel async fetches in `CaptainContextBuilder` — `EpisodicRecall + liveHR + restingHR` all `async let`-fanned-out so the cold path is bounded by `max()` not `sum()`. The reader produces 7 inferred moods (relaxed/focused/excited/stressed/postEffort/windingDown/unknown) with arousal classification (`calm`/`neutral`/`activated`/`highlyAroused`) and confidence in [0.6, 1.0]. **Privacy contract: only the categorical mood and a tone directive ride the prompt to the cloud — the raw HR numbers never leave the device.** | `hrMood` |

The 14 layers + the existing 7 prompt layers + the §41 + §48 gates = **14-layer brain on top of a 15-layer prompt composer**. The brief is the executive summary; the prompt is the full presentation; the gate is the receipt.

### 35.4 The Authority Chain (§50)

When two layers disagree about what the next reply should look like, the angle picker resolves them via a fixed 9-level chain. Documented as a doc-comment block in `CognitiveReasoner.pickAngle(...)` and mirrored as a § marker block in `PromptComposer.layerReasoningBrief`'s header so a maintainer reading either file sees the same authority order:

```
1. EXPLICIT USER FEEDBACK    coherence.userIsFrustratedWithCaptain → repair
2. PHYSIOLOGY (HR mood)      stressed/postEffort/windingDown/excited/relaxed → angle
3. RECENT ACTIVITY           freshness == .veryFresh → recovery
4. BEHAVIOURAL STAGE         relapse → gentle, contemplation → factual
5. ACUTE PATTERNS            lowSleepStreak / fatigueAccumulation → gentle
6. TEXT EMOTION              stressed → grounding, tired → gentle, motivated+lever → push
7. POSITIVE PATTERNS         personalBest → celebrate, streakAtRisk → push
8. RHYTHM PATTERNS           monotony / consistencyImproving / loadStreak → proactive
9. DEFAULT                   factual
```

Higher tier always wins on conflict. Within a tier, left-to-right resolves ties. The single `pickAngle` function is the *only* place in the codebase where these signals get combined into a final angle — no other layer overrides this decision, and the §41 quality gate's `angleViolation` check enforces the resolution post-generation by flagging replies whose tone reads inconsistent with the chosen angle.

### 35.5 Six audit rounds — closing cross-layer gaps

After the initial implementation, six progressively-deeper audits found **20 inter-layer integration issues** that compiled cleanly and shipped reasonable behavior individually, but produced subtle silent contradictions when layers disagreed. Each round's fixes are tagged with `§50 Round N Fix X` doc-comments at the call site so future maintainers can read the audit history inline:

| Round | Issues Fixed | Highlights |
|---|---|---|
| 1 | 7 | `pickAngle` integrates HR + behavioral stage; HabitDetector calibrated by `profileLens`; PredictiveIntent HR-aware; Quality Gate `angleViolation` check; brief subsection reorder; authority-chain doc block. |
| 2 | 3 | `ReasoningBrief.isEmpty` checks all 17 fields (was 7); workout specialist defers intensity ceiling to the brief's angle (`clampIntensityToAngle`); `composeThesis` injects an `upstreamCauseFragment` when HR or stage drove the angle. |
| 3 | 6 | `pickAngle` enumerates all 7 HR moods (was 3); de-dup emotion detection (read from `coherence.latestEmotion`, don't re-detect); `noSpecificNumber` skips ultra-short replies (≤5 words); corrective regen prefix carries the thesis; `causeSpecificGentleHook` differentiates by cause; **invariant validator** with 4 contradiction checks. |
| 4 | 4 | `shouldHaveNumber` skips greetings (`"شلونك؟"` doesn't warrant a number even with rich data); `hardAvoidances` (refusals) vs `softAvoidances` (just-completed) split; invariants expanded to 7 checks; `CausalChainBuilder` accepts behavioral stage and shrinks chain depth for relapse / contemplation. |
| 5 | 5 | Quality Gate's `usedAvoidedFamily` carries `AvoidSeverity` so soft hits are warnings (-0.25) and hard hits are critical (-0.5); `CaptainContextBuilder` parallelizes the three async fetches (was sequential, now `async let`-fanned); specialist `gentle` clamp softened from `→ light` to `→ moderate`; opening-hook priority reordered (coherence frustration > HR); cause-fragment coverage extended to `excited / proactive / contemplation`. |
| 6 | 4 | `CaptainCustomizationKeys` extracted as the single source of truth for UserDefaults keys (lens + ViewModel both reference it); `HabitDetector` caps patterns at 1 for relapse / contemplation users; **causal chain skipped when workout specialist will fire** (one canonical explanation, not two overlapping ones); HR fallback baseline scales by age bracket (60 BPM youth → 70 BPM senior). |

Total: **20 inter-layer fixes** across 6 rounds, each a small surgical change with no API breaks and no test regressions.

### 35.6 The Invariant Validator

The `ReasoningBrief.brokenInvariants()` method (Round 3 Fix V, expanded by Round 4 Fix γ) runs **7 cross-layer contradiction checks** the moment the brief is assembled. In DEBUG it `assertionFailure`s — the simulator crashes on contradiction so the dev sees the bug immediately. In RELEASE it's a no-op so a brain inconsistency never crashes a user's session; production telemetry would catch repeated occurrences instead.

| # | Invariant | Why it matters |
|---|---|---|
| 1 | Specialist intensity vs protection angle | `repair` / `grounding` angles + `hard` / `maximal` intensity = body-protection failure |
| 2 | Callback suggests an avoided family | A "ready phrase" that proposes walking when walking is on the avoid-list |
| 3 | Stressed HR + push/celebrate angle | Body says backoff, prompt says push — the bug-of-record's exact shape |
| 4 | Relapse stage + push/celebrate | Returning user being hyped at = patronizing |
| 5 | postEffort HR + angle ≠ recovery | Body is in recovery; the prompt should reflect that |
| 6 | contemplation stage + pushForward | Exploring user being pressured |
| 7 | push angle without personal-best evidence | Push needs justification, not vibes |

These exist *because* across 5 audit rounds we found cases where the angle picker accidentally landed in a state that contradicted another layer's signal. The validator turns "we should never produce this combo" from a written-down rule into a compiler-fast assertion.

### 35.7 Keyboard-occlusion fix (commit `83ef407`)

Surfaced mid-pass: when the user had the keyboard open and a Captain reply arrived, the new bubble landed *behind* the keyboard, requiring the user to dismiss the keyboard to see what was said. The pre-existing `.scrollDismissesKeyboard(.immediately)` only fires for *user-initiated* scrolls, not programmatic ones, so the auto-scroll triggered by the new message ran against pre-keyboard scrollview geometry and missed the new bubble's frame.

Three coordinated changes in [`CaptainChatView`](AiQo/Features/Captain/CaptainChatView.swift):

1. **Hoist `@FocusState`** from `ChatComposerBar` up to the parent `CaptainChatView` so the parent observes keyboard show/hide transitions. The composer still owns the underlying state via `FocusState<Bool>.Binding`.
2. **`Task.yield` before scroll** on `messages.count` change — gives SwiftUI a layout pass against the new bubble's real frame before the scroll fires.
3. **320 ms delayed scroll on focus → true** matches the system keyboard animation duration so the scroll lands against the post-keyboard safe-area inset, not the pre-keyboard one.

Also relaxed `scrollDismissesKeyboard(.immediately)` to `.interactively` so a user-initiated drag still dismisses but programmatic scrolls don't fight the keyboard mid-animation.

### 35.8 Files

**New brain code (15 files, ~3,000 LOC):**

- [`AiQo/Features/Captain/Brain/01_Sensing/HRMoodReader.swift`](AiQo/Features/Captain/Brain/01_Sensing/HRMoodReader.swift) — §49 mood reader + age-aware fallback baseline
- [`AiQo/Features/Captain/Brain/03_Reasoning/RecentActivitySnapshot.swift`](AiQo/Features/Captain/Brain/03_Reasoning/RecentActivitySnapshot.swift) — §33 + family classifier
- [`AiQo/Features/Captain/Brain/03_Reasoning/ConversationCoherenceAnalyzer.swift`](AiQo/Features/Captain/Brain/03_Reasoning/ConversationCoherenceAnalyzer.swift) — §34 + hard/soft avoid split
- [`AiQo/Features/Captain/Brain/03_Reasoning/CognitiveReasoner.swift`](AiQo/Features/Captain/Brain/03_Reasoning/CognitiveReasoner.swift) — §37 + §50 authority chain + invariants
- [`AiQo/Features/Captain/Brain/03_Reasoning/HabitDetector.swift`](AiQo/Features/Captain/Brain/03_Reasoning/HabitDetector.swift) — §38 stage-capped
- [`AiQo/Features/Captain/Brain/03_Reasoning/UserProfileLens.swift`](AiQo/Features/Captain/Brain/03_Reasoning/UserProfileLens.swift) — §39 + `CaptainCustomizationKeys`
- [`AiQo/Features/Captain/Brain/03_Reasoning/MicroInsightGenerator.swift`](AiQo/Features/Captain/Brain/03_Reasoning/MicroInsightGenerator.swift) — §40
- [`AiQo/Features/Captain/Brain/03_Reasoning/PersonalityStyleVector.swift`](AiQo/Features/Captain/Brain/03_Reasoning/PersonalityStyleVector.swift) — §42
- [`AiQo/Features/Captain/Brain/03_Reasoning/CausalKnowledgeGraph.swift`](AiQo/Features/Captain/Brain/03_Reasoning/CausalKnowledgeGraph.swift) — §43 + stage-aware depth
- [`AiQo/Features/Captain/Brain/03_Reasoning/BehavioralStageEngine.swift`](AiQo/Features/Captain/Brain/03_Reasoning/BehavioralStageEngine.swift) — §44 TTM detector
- [`AiQo/Features/Captain/Brain/03_Reasoning/PredictiveIntentEngine.swift`](AiQo/Features/Captain/Brain/03_Reasoning/PredictiveIntentEngine.swift) — §45 HR-aware
- [`AiQo/Features/Captain/Brain/03_Reasoning/EpisodicRecallProvider.swift`](AiQo/Features/Captain/Brain/03_Reasoning/EpisodicRecallProvider.swift) — §46 cross-session recall
- [`AiQo/Features/Captain/Brain/03_Reasoning/SpecialistReasoners.swift`](AiQo/Features/Captain/Brain/03_Reasoning/SpecialistReasoners.swift) — §47 + angle clamp
- [`AiQo/Features/Captain/Brain/04_Inference/ResponseQualityGate.swift`](AiQo/Features/Captain/Brain/04_Inference/ResponseQualityGate.swift) — §41 + severity + angle adherence
- [`AiQo/Features/Captain/Brain/04_Inference/IraqiDialectScorer.swift`](AiQo/Features/Captain/Brain/04_Inference/IraqiDialectScorer.swift) — §48 MSA-vs-Iraqi

**Modified (8 files):**

- [`AiQo/Features/Captain/Brain/01_Sensing/CaptainHealthSnapshotService.swift`](AiQo/Features/Captain/Brain/01_Sensing/CaptainHealthSnapshotService.swift) — `fetchRestingHeartRate()` + `fetchLiveHeartRate()` (5-min lookback, 2s timeout each)
- [`AiQo/Features/Captain/Brain/02_Memory/ConversationThread.swift`](AiQo/Features/Captain/Brain/02_Memory/ConversationThread.swift) — §35 structured-tag prompt summary
- [`AiQo/Features/Captain/Brain/03_Reasoning/CaptainContextBuilder.swift`](AiQo/Features/Captain/Brain/03_Reasoning/CaptainContextBuilder.swift) — `recentActivity` + `coherenceTags` + `episodicRecall` + `hrMood` + `reasoningBrief` fields, parallel async fetches, lens-aware HR baseline
- [`AiQo/Features/Captain/Brain/03_Reasoning/CognitivePipeline.swift`](AiQo/Features/Captain/Brain/03_Reasoning/CognitivePipeline.swift) — `.general` intent now weights `workout_history` at 2.4 (was absent — root cause of the bug-of-record's "what should we do?" turn missing the recent walk)
- [`AiQo/Features/Captain/Brain/04_Inference/BrainOrchestrator.swift`](AiQo/Features/Captain/Brain/04_Inference/BrainOrchestrator.swift) — `applyQualityGate(...)` post-generation + `augmentRequestWithCorrection(...)` regen path with thesis-enriched prefix
- [`AiQo/Features/Captain/Brain/04_Inference/PromptComposer.swift`](AiQo/Features/Captain/Brain/04_Inference/PromptComposer.swift) — `layerRecentActivity` (§33), `layerConversationCoherence` (§34), `layerReasoningBrief` (§37–§47, 5-block hierarchy), §50 authority-chain doc block, REPLY QUALITY block in identity layer
- [`AiQo/Features/Captain/CaptainViewModel.swift`](AiQo/Features/Captain/CaptainViewModel.swift) — `Keys` typealiased to `CaptainCustomizationKeys`, conversation forwarded to `buildContextData(conversation:)`
- [`AiQo/Services/Analytics/AnalyticsEvent.swift`](AiQo/Services/Analytics/AnalyticsEvent.swift) — `captainQualityGateViolation(code:isCritical:score:)` + regen succeeded/rejected events
- [`AiQo/Features/Captain/CaptainChatView.swift`](AiQo/Features/Captain/CaptainChatView.swift) — keyboard-occlusion fix (separate commit `83ef407`)

**New tests (10 files, ≈64 unit tests):**

- [`AiQoTests/ConversationCoherenceAnalyzerTests.swift`](AiQoTests/ConversationCoherenceAnalyzerTests.swift) — including `testReproducesScreenshotBugSurfacesWalkAvoid` which encodes the exact bug-of-record dialogue
- [`AiQoTests/CognitiveReasonerTests.swift`](AiQoTests/CognitiveReasonerTests.swift)
- [`AiQoTests/PersonalizedReasoningTests.swift`](AiQoTests/PersonalizedReasoningTests.swift) — `HabitDetector` + `UserProfileLens` + `MicroInsightGenerator`
- [`AiQoTests/ResponseQualityGateTests.swift`](AiQoTests/ResponseQualityGateTests.swift) — including the screenshot reproduction
- [`AiQoTests/PersonalityStyleVectorTests.swift`](AiQoTests/PersonalityStyleVectorTests.swift)
- [`AiQoTests/CausalKnowledgeGraphTests.swift`](AiQoTests/CausalKnowledgeGraphTests.swift)
- [`AiQoTests/BehavioralStageEngineTests.swift`](AiQoTests/BehavioralStageEngineTests.swift)
- [`AiQoTests/PredictiveIntentEngineTests.swift`](AiQoTests/PredictiveIntentEngineTests.swift)
- [`AiQoTests/IraqiDialectScorerTests.swift`](AiQoTests/IraqiDialectScorerTests.swift)
- [`AiQoTests/HRMoodReaderTests.swift`](AiQoTests/HRMoodReaderTests.swift)

Zero `pbxproj` edits across all 33 files — Xcode 16 `PBXFileSystemSynchronizedRootGroup` picks up everything automatically.

### 35.9 Build & verification

`xcodebuild -project AiQo.xcodeproj -scheme AiQo -sdk iphonesimulator -destination 'platform=iOS Simulator,name=iPhone 17' -configuration Debug build` returns `** BUILD SUCCEEDED **` after every round, with **zero errors and zero warnings on touched files**. The pre-existing SourceKit "Cannot find type X in scope" diagnostics on `HybridBrainRequest` / `AppLanguage` / `CaptainContextData` etc. are unaffected — same root cause as §32.5 / §33.9 / §34.5 (SwiftUI indexer lag against the synchronized-group target).

A pre-existing `import Combine` regression in [`AiQo/Features/Kitchen/CookModeView.swift`](AiQo/Features/Kitchen/CookModeView.swift) (Timer's `autoconnect()` couldn't resolve under fresh-state builds) was fixed inline as part of the kitchen WIP commit (`85de575`); not related to the brain refactor.

Test execution requires bypassing a pre-existing project setup issue: the `AiQoWatch Watch App` Info.plist is missing the `WKWatchKitApp` / `WKApplication` key, which causes the simulator install validator to reject the parent app bundle. Running tests through the iPhone 17 simulator with the standard scheme produces an "Unable to install AiQo" error from the Watch sub-app validator. The fix is project-config (out of scope for §35); the Swift code itself compiles cleanly. Tests can be run via `xcodebuild test -derivedDataPath /tmp/aiqo-isolated` with a separate DerivedData path that sidesteps the IDE's concurrent build lock — that's how the brain refactor was empirically verified.

### 35.10 Three commits, one branch, pushed

Branch `brain-refactor/p-fix-dev-override` on `git@github.com:mraad500/AiQo.git`, pushed 2026-05-10:

| SHA | Subject | Files |
|---|---|---|
| `d955c04` | feat(captain-brain): comprehensive cognitive brain refactor (§33–§50) | 33 |
| `83ef407` | fix(captain-chat): keep latest reply visible when keyboard is open | 1 |
| `85de575` | chore: save in-progress kitchen + gym-plan work | 19 |

The brain commit's body is the canonical change-summary; cross-reference §35.3–§35.8 for details. The keyboard fix is split into its own commit because it's a UX bug discovered mid-pass and warrants independent revert if needed. The kitchen WIP commit is the user's pre-existing untracked work plus the one-line `import Combine` fix that unblocked fresh-state builds — it's a snapshot, not a feature.

### 35.11 Follow-ups

- **Toggle `CAPTAIN_QUALITY_REGEN_ENABLED` in production after telemetry shape stabilizes.** The Quality Gate emits `captain_quality_gate_violation` events for every detected failure today regardless of whether regen fires; once we've seen a week of production telemetry and confirmed the violation rate is genuinely low (i.e. the brain's pre-prompt synthesis is doing its job), flipping the regen flag to `<true/>` adds a single corrective pass on critical hits as a safety net. Latency + cost trade-off tracked in §28.7.
- **Migrate the §41 quality-gate analytics events to a Supabase `captain_quality_gate` table** (deferred from §34.6's `CaptainMetricsCounter` migration). Same idiom: namespaced UserDefaults works for TestFlight, but a real table indexed by `(violation_code, severity, score_bucket)` is the proper home for production dashboards.
- **Build out `NutritionSpecialistReasoner` + `SleepSpecialistReasoner`.** The §47 sum-type architecture has reserved cases for both. Phase-12 shipped only the workout specialist as the proof-of-concept; nutrition is the natural next domain (it has its own intent classifier + a Kitchen surface that produces meal plans), and sleep follows after.
- **Snapshot test for `ReasoningBrief.brokenInvariants()` returning `[]`** across the typical-input space. The 7 invariants exist in DEBUG asserts but have no positive coverage today — we know they fire on contradictions, we don't know they *don't* false-fire on valid combos. A combinatorial test over (angle × hrMood × stage × hasAvoidances × hasFreshActivity) would lock that down.
- **Lift `CaptainCustomizationKeys` to read from `UserDefaults` via a typed accessor** (`UserDefaults.captain.age` etc.). The string-keyed `defaults.string(forKey: ...)` pattern works but a typed wrapper would catch a future "key changed but reader didn't" bug at compile time.
- **Add `EpisodeConsolidator` (deferred §46.bis)** — at end of significant conversations, generate a structured "session digest" that compresses 20+ messages into a 2-3 sentence high-level summary, and surface that as the recall block instead of (or alongside) raw episode quotes. The current §46 surfaces recent quotes verbatim; consolidation would surface *themes*.
- **Wire `behavioralStage` into `MicroInsightGenerator` + `CausalChainBuilder`** for the few remaining stage-blind paths. The Round 4 + Round 6 fixes covered the load-bearing ones (HabitDetector, CausalChainBuilder, predictor); the micro-insight generator still uses fixed thresholds for "best sleep delta ≥ 0.7h" etc. that could be stage-aware.
- **Snapshot the byte-identity contract for `ReasoningBrief` rendering** like §34.6's v2-app-knowledge contract. Same value across calls = caching opportunity at the Edge Function layer.
- **Resolve the WatchKit Info.plist install gate** so `xcodebuild test` against the iPhone scheme works without the isolated-DerivedData workaround. Project-config issue, not Swift.
- **Privacy review for the §49 HR mood directive.** The current contract — only the categorical mood + tone directive ride to the cloud, never raw HR — is documented in the file header but not in the privacy manifest. Worth a §27-style audit pass before TestFlight to confirm the data-collected disclosure stays accurate.

---

## 36. Plan Feature World-Class Upgrade — Live Runner, Templates, Insights, Stats (2026-05-10)

*Major feature pass on the Gym → Club → Plan surface, executed end-to-end on branch `brain-refactor/p-fix-dev-override` and bundled into commit [`85de575`](https://github.com) (`chore: save in-progress kitchen + gym-plan work`) alongside the brain-refactor §35 work and concurrent Kitchen WIP. Lifts the Plan tab from a minimal "type weight + goal → Captain replies → pin → flat list" loop into a Strong / Future / Centr-tier surface: heuristic exercise metadata, structured chip-driven intake, a full-screen live workout runner with set-level tracking and auto rest timer, six curated quick-start templates, a four-card weekly stats hero, an exercise detail sheet with form cues and alternatives, completion celebration with confetti, and a shareable-text export. Core `WorkoutPlan` / `Exercise` decoder invariants are untouched; the Brain inference layer and the §35 14-layer reasoning brain are untouched; round-trip persistence is backwards-compatible with legacy `WorkoutTask.title` rows so nothing already pinned breaks. Zero `pbxproj` edits — Xcode 16 file-system-synchronized groups pick up all seven new Swift files automatically. Zero new outbound HTTP calls — all metadata derivation is on-device heuristics over exercise names; the Captain chat path is unchanged and still routes through `globalBrain.sendMessage(...)`. The §32 / §34 app-knowledge prompt layer is unaffected — the new helpers do not feed Captain's prompt.*

### 36.1 The problem

Pre-upgrade, the Plan tab was the thinnest surface in the app:

- The intake was a free-form text prompt (`"حتى أبني خطة تمرين شخصية، اكتب وزنك الحالي وهدفك مثل: 95 كيلو وتنشيف."`) — users had to know what shape of input the model wanted.
- The plan-ready preview card was a static list of names, sets, and reps with no metadata or interactivity.
- The pinned plan in the daily record showed up as a flat row of `"<name> - <sets> sets - <reps>"` strings — no muscle group, no equipment, no estimated duration, no completion semantics beyond a single global checkbox per row.
- There was no way to *run* a workout — once pinned, exercises lived as static text. Set-by-set tracking, rest timers, and total session timing all lived in the separate `LiveWorkoutSession` (cardio / Apple-Watch path) but never wired to a pinned plan.
- There was no curated discovery — every plan went through Captain.
- There was no progress surface — no week view, no streak, no completion ratio.

Pinning a plan stored each exercise as a free-form string; once pinned, the *structured* shape (sets count, reps/duration as separate fields) was lost. The card on the dashboard had no way to show muscle-group icons, equipment chips, or per-set completion because the data was already textually flattened.

### 36.2 Heuristic exercise insights — `WorkoutPlanInsights.swift`

[`AiQo/Features/Gym/Club/Plan/WorkoutPlanInsights.swift`](AiQo/Features/Gym/Club/Plan/WorkoutPlanInsights.swift) (498 LOC, new) is the entirety of the metadata-derivation layer. It converts an `Exercise` (whose only structured fields are `name`, `sets`, `repsOrDuration`) into a fully-typed `ExerciseInsights` value:
...
Three enums anchor the model:

| Enum | Cases | Carries |
|---|---|---|
| `WorkoutMuscleGroup` | `chest`, `back`, `legs`, `glutes`, `shoulders`, `arms`, `core`, `fullBody`, `cardio`, `mobility` | `icon` (SF Symbol), `arabicLabel` / `englishLabel`, `accent: Color` |
| `WorkoutEquipment` | `bodyweight`, `dumbbell`, `barbell`, `machine`, `band`, `kettlebell`, `cable` | same shape |
| `WorkoutDifficulty` | `beginner`, `intermediate`, `advanced` | label pair + `accent: Color` |

Classification lives in a private `ExerciseClassifier` enum. Each function takes the lowercased exercise name and walks an Arabic-and-English keyword table:
...
static func muscleGroup(for normalizedName: String) -> WorkoutMuscleGroup {
    ...
    // ... 9 more groups, fallthrough .fullBody
}
```

Equipment falls back to `.bodyweight` when no marker matches — the safe assumption since Captain Hamoudi defaults to no-equipment plans for users who haven't declared a gym setup.

`estimatedSetSeconds(for repsOrDuration:muscle:)` parses the rep/duration string ad-hoc:
- `"60 ثانية"` / `"30 sec"` → seconds literal
- `"3 دقائق"` / `"5 min"` → minutes × 60
- `"12 تكرار"` / `"10 reps"` → reps × seconds-per-rep weighted by muscle (compound lifts get 3.5s, accessory work 2.8s, mobility 4.0s, cardio/full-body 2.4s)

The rep-to-second mapping is intentionally generous on the upper end so the dashboard's "Approx" badge skews high rather than promising a 5-minute workout that takes 8.

Rest seconds derive from `(muscle, equipment)` pairs — barbell compounds get 90s, accessory bodyweight gets 50s, mobility gets 20s. Plan-level `WorkoutPlanInsights` aggregates over the exercise list, sorts muscle and equipment by frequency for the badge strip, and assigns difficulty by total set count + max sets seen on any single exercise:
...
`formCues(for:language:)` and `alternatives(for:language:)` keep two parallel switch ladders (Arabic and English) so the cues/alternatives never machine-translate. Arabic cues lean on culturally appropriate brevity ("خلي الكتف ثابت ولا يطلع للأمام") rather than literal renderings of English coaching jargon.

### 36.3 Round-trip persistence — `ExerciseSerialization`

The pre-upgrade `WorkoutPlanMemoryStore.savePlan(...)` flattened each exercise into a localized title via `String(format: L10n.t("gym.plan.taskFormat"), name, sets, repsOrDuration)`, dropping the structured shape on the floor. The new persistence path encodes the `Exercise` reversibly inside `WorkoutPlanFlowViews.swift`:
...
`parse(taskTitle:)` round-trips the prefix; on a missing prefix it falls back to `parseLegacyTitle(_:)` which best-effort splits on `" - "` / `" — "` / `" – "` and digit-extracts the sets count from the middle segment. Legacy `WorkoutTask.title` rows that already exist on a user's device thus continue to render — they just don't carry the structured data into the new dashboard's `ActivePlanCard` (the `displayTitle` helper falls through to the raw legacy string).

`displayTitle(taskTitle:)` formats the round-tripped exercise as `"<name> — <sets> × <reps>"` (em-dash + multiplication sign) for the history snapshot rows on the dashboard. The `AIQEX1‖` magic prefix is intentionally non-printable in normal UI contexts (`‖` U+2016 Double Vertical Line is a separator, not a typographic mark) — if any code path slips and surfaces a raw title, the prefix makes the bug obvious in the screenshot.

### 36.4 Active plan card — progress ring + start button — `WorkoutPlanCards.swift`

[`AiQo/Features/Gym/Club/Plan/WorkoutPlanCards.swift`](AiQo/Features/Gym/Club/Plan/WorkoutPlanCards.swift) (718 LOC, new) hosts three rendering surfaces shared between the dashboard and the chat: `ActivePlanCard`, `PendingPlanPreviewCard`, and `WeeklyProgressStrip`.

`ActivePlanCard` replaces the pre-upgrade `CaptainLiveWorkoutPlanCard` with five stacked sections:

1. **Header** — animated `Circle().trim(from:to: progress)` ring around the icon. Stroke is mint→blue gradient pre-completion, mint→green gradient when `progress >= 1.0`. Icon flips from `sparkles` to `checkmark` on completion. The rotation is `-90°` so 0 starts at 12 o'clock.
2. **Badge strip** — four pills: duration (`{prettyDuration} min`), difficulty + icon (leaf/flame/bolt by tier), top muscle group, top equipment. Each pill carries its own `tint` accent and a `LinearGradient` fill with a 35%-alpha shadow.
3. **Progress row** — `"Today's progress"` label + `{done}/{total}` counter + 8pt-tall `Capsule().fill(LinearGradient(...))` bar bound to `progress` with a `.spring(response: 0.45, dampingFraction: 0.85)` animation.
4. **Start workout button** — full-width gradient capsule. Label flips to `"Run it again"` once `allDone` is true; gradient flips from mint→blue to lime→mint to signal completed state.
5. **Exercise list** — per-row `ActivePlanExerciseRow` with a `Button` checkbox (calls `onToggleCompletion(index:)`) plus a tappable body (calls `onTapExercise(exercise:)` to surface `ExerciseDetailSheet`). Completed rows strike through, drop opacity to 0.78, and re-tint their border with the muscle accent color. Row haptics: `UISelectionFeedbackGenerator` on each toggle.

`PendingPlanPreviewCard` is the chat-side equivalent — same metadata but compressed into a tap-to-explore preview shown above the "Pin" button. Tapping any row of the first 4 exercises opens the detail sheet; an `+N more exercises` row collapses the tail.

`WeeklyProgressStrip` renders 7 day-pills (oldest → newest, ending today). Each pill shows the day's locale-formatted `EEE` short label, a 32pt circle with a fill tint that grades `.completed` (mint) → `.partial` (amber) → `.today` (blue) → `.upcoming` (gray), and a small mint underline marker on today. A trim-stroke overlay encodes the day's `completionRatio` as a partial ring around the pill. The header line carries a streak badge — orange `🔥 N-day streak` when the trailing run is non-zero, gray `Start a new streak` otherwise.

### 36.5 Live Plan Workout Runner — `PlanWorkoutRunner.swift`

[`AiQo/Features/Gym/Club/Plan/PlanWorkoutRunner.swift`](AiQo/Features/Gym/Club/Plan/PlanWorkoutRunner.swift) (784 LOC, new) is the screen the upgrade's identity hangs on. Presented as a `.fullScreenCover` from `WorkoutPlanDashboard.showRunner`. The view tracks per-exercise set completion separately from the dashboard's per-exercise checkbox — sets fold up into the dashboard's "exercise complete" boolean only when `setsCompleted[index] >= currentExercise.sets`.
...
`tick()` runs every second. It increments `elapsedSeconds` (unless paused or completion overlay is up), decrements `restRemaining` while resting, and fires a `UIImpactFeedbackGenerator(style: .medium)` haptic at rest end. On `allDone` it triggers `triggerCompletion()` which lights the success haptic (`UINotificationFeedbackGenerator().notificationOccurred(.success)`) and `withAnimation(.spring(...))` reveals the completion overlay.

The screen has six visual zones:

1. **Top bar** — close button, plan title + "Live workout" overline, monospaced `MM:SS` elapsed pill.
2. **Progress header** — `"Exercise X of Y"` overline + total `{setsDone}/{setsTotal} sets` counter + 8pt mint→blue progress capsule bound to `progressFraction = totalSetsDone / totalSetsAcross`.
3. **Exercise focus card** — large 56pt gradient circle with the muscle's SF Symbol, exercise name (22pt heavy), three metric pills (sets / reps / rest), and a quick-cue surface that pulls `insights.formCues.first` into a yellow-tinted callout under a `lightbulb.fill` glyph. Background gradient is muscle-accent → systemBackground.
4. **Sets tracker** — N circular 50pt buttons numbered `1..sets`. The current "next" set scales 1.06×, gets a 2.5pt accent stroke and a colored shadow; completed sets fill with the muscle accent and show a `checkmark` with `.symbolEffect(.bounce, value: isComplete)`. Tapping the *current* set increments `setsCompleted` and either auto-advances after 0.45s if the exercise is done, or starts the rest timer.
5. **Rest card** — appears only when `isResting`. 38pt monospaced countdown, 6pt blue→purple progress capsule whose width is `1.0 - (restRemaining / restSeconds)`, and a "Skip" pill that zeroes `restRemaining` immediately. Light haptic on skip.
6. **Up next strip** — bottom 3 rows showing the next exercises with their muscle-tinted icons.

Bottom action bar holds two pills: `Pause/Resume` (toggles `isPaused`, blocks the elapsed counter and the rest decrement) and `Skip/Next` (advances to the next exercise; label flips to "Next" once all sets on the current exercise are done).

**Completion celebration** is the screen's signature flourish. On `triggerCompletion()`:

- A black-45%-alpha overlay fills the screen.
- A 96pt mint→blue gradient circle holds a 40pt `checkmark` with a 22pt-radius mint glow.
- Headline `"🎉 You crushed it!"` (or `"🎉 خلصت بطل!"`).
- Stats line `"{exercises} exercises • {sets} sets • {MM:SS}"`.
- A capsule "Done" button calls `onCompleteAll()` — the dashboard handler marks every `WorkoutTask` as completed in the SwiftData record and reloads.
- A `ConfettiView` overlay drops 60 randomized rectangle pieces from y=-40 to y=height+30 over 1.6-2.8s with random delays. Each piece is sized 6-12pt × 0.45-aspect with one of five brand accents and a random rotation. The view is `.allowsHitTesting(false)` so the user can dismiss without fighting falling confetti.

Haptics map cleanly to user intent:

| Event | Generator | Style |
|---|---|---|
| Tap a set checkbox | `UISelectionFeedbackGenerator().selectionChanged()` | — |
| Auto-advance to next exercise | `UIImpactFeedbackGenerator` | `.medium` |
| Rest timer reaches 0 | `UIImpactFeedbackGenerator` | `.medium` |
| Skip rest | `UIImpactFeedbackGenerator` | `.light` |
| All exercises complete | `UINotificationFeedbackGenerator` | `.success` (fires once via `didFireSuccessHaptic` guard) |

`import Combine` is required at the top — without it, `Timer.publish(every:...).autoconnect()` fails with `instance method 'autoconnect()' is not available due to missing import of defining module 'Combine'`. This was the only real compile error during the build; SourceKit's "Cannot find type X in scope" cascade on the new files cleared once swiftc indexed.

### 36.6 Quick-start template catalog — `WorkoutTemplateLibrary.swift`

[`AiQo/Features/Gym/Club/Plan/WorkoutTemplateLibrary.swift`](AiQo/Features/Gym/Club/Plan/WorkoutTemplateLibrary.swift) (326 LOC, new) ships six hand-curated plans that cover the full duration spectrum and the most common goals:

| ID | Title (Ar / En) | Duration | Exercises | Tint |
|---|---|---|---|---|
| `energy.5min` | صحوة 5 دقائق / 5-Min Wake-Up | 5 min | 4 (breathing, joint prep, jumping jacks, squats) | warm orange |
| `mobility.10min` | مرونة 10 دقائق / 10-Min Mobility | 10 min | 5 (breathing, cat-cow, hip opener, hamstring, shoulder/chest) | lavender |
| `core.15min` | كور 15 دقيقة / 15-Min Core | 15 min | 5 (plank, side plank, mountain climbers, Russian twist, leg raise) | mint |
| `fullBody.20min` | جسم كامل 20 دقيقة / 20-Min Full Body | 20 min | 6 (warmup, squat, push-up, lunge, inverted row, plank) | sky blue |
| `cardio.30min` | كارديو 30 دقيقة / 30-Min Cardio | 30 min | 6 (warmup, jog, burpee, mountain climbers, jump rope, cooldown) | coral |
| `strength.45min` | قوة 45 دقيقة / 45-Min Strength | 45 min | 7 (warmup, squat, press, row, hip thrust, side plank, stretch) | violet |

Each `WorkoutTemplate` carries parallel Arabic and English `Exercise` arrays — when the user taps a card, `template.plan(language:)` returns the locale-appropriate variant. The catalog is a `static let all: [WorkoutTemplate]` constant in `enum WorkoutTemplateCatalog` — adding a 7th template is a single array push.

`QuickStartTemplatesStrip` renders the catalog as a horizontal `ScrollView` of 175pt-wide cards under the `"Quick start"` / `"بدء فوري"` header. Each card shows: 36pt gradient circle with the template's icon, `{minutes} min` overline + `{count} moves`, the localized title, the descriptor line, and a "Pin now" pill. Tapping any card sets `pendingTemplate` on the dashboard, which surfaces a confirmation `.alert(...)` — explicit confirmation is necessary because pinning replaces the day's existing plan entirely.

The strip lives between the active-plan card and the compliance card on the dashboard, giving newcomers an instant on-ramp that doesn't require talking to the Captain.

### 36.7 Weekly stats hero — `PlanWeeklyStats.swift`

[`AiQo/Features/Gym/Club/Plan/PlanWeeklyStats.swift`](AiQo/Features/Gym/Club/Plan/PlanWeeklyStats.swift) (197 LOC, new) is the analytics surface. `PlanWeeklyStats` is a value type aggregating four trailing-7-day metrics:
...
`WorkoutPlanMemoryStore.fetchWeeklyStats(modelContext:language:)` (new extension method) walks `AiQoDailyRecord`s in the trailing 7-day window, sums `task.isCompleted` counts, and feeds completed exercises into `exercise.insights(language:).estimatedSeconds` to derive `totalMinutes`. Legacy task titles that fail the round-trip parse get a 90-second fallback so the minutes counter never silently zeros on old data.

The streak walk is intentionally backward-only: starts at today, decrements one day at a time, requires `record.workouts.contains(where: \.isCompleted)`, breaks on the first miss. A user who skipped yesterday but trained today gets a 1-day streak, not zero — the active day always counts.

`PlanWeeklyStatsHero` renders the four metrics in a single horizontal row of mini-cards, each with its own tint: mint sessions, blue minutes, amber completion %, coral streak. Completion-percent gets a label suffix derived from the percentage band — `elite` ≥ 90%, `strong` ≥ 70%, `solid` ≥ 40%, `ramp up` otherwise — so the hero stays positive even on a slow week. The hero is hidden when `totalSessions == 0` so first-launch users don't see an empty week-empty surface; the empty-state feature card carries the whole "what you'll get" pitch on its own (see §36.10).

### 36.8 Captain chat intake + refinement chips — `WorkoutPlanIntakeChips.swift`

[`AiQo/Features/Gym/Club/Plan/WorkoutPlanIntakeChips.swift`](AiQo/Features/Gym/Club/Plan/WorkoutPlanIntakeChips.swift) (377 LOC, new) replaces the free-form "type your weight + goal" intake with a four-step chip flow. Four enums define the selection space:

| Step | Enum | Cases |
|---|---|---|
| Goal | `PlanIntakeGoal` | `buildMuscle`, `cutFat`, `generalFitness`, `mobility`, `eventPrep` |
| Level | `PlanIntakeLevel` | `beginner`, `intermediate`, `advanced` |
| Duration | `PlanIntakeDuration` (Int-backed) | `15`, `30`, `45`, `60` |
| Equipment | `PlanIntakeEquipment` | `bodyweight`, `dumbbells`, `fullGym` |

`PlanIntakeSelection.composedMessage(language:)` synthesizes a structured Arabic or English request from the four picks:

```swift
// Arabic:
"أبني خطة تمرين شخصية: هدفي {goal}، مستواي {level}، عندي {duration} باليوم،
 والمعدّات المتاحة {equipment}. اعطني خطة بتمارين واضحة بمجاميع وعدّات."

// English:
"Build me a personalized workout plan. Goal: {goal}. Level: {level}.
 Available time: {duration}. Equipment: {equipment}.
 Give a clean plan with sets and reps."
```

The composed message is sent through the existing `globalBrain.sendMessage(_:context: .gym)` — the brain inference layer is untouched. Captain Hamoudi reads it through the same prompt path as any free-form message, which means the §35 14-layer reasoning brain processes the intake exactly like a typed message, and the §32 / §34 app-knowledge layer still slots in.

The chip surface only appears when `!hasUserMessages && globalBrain.currentWorkoutPlan == nil && !globalBrain.isLoading`. Once the user types a free-form message *or* completes the chip flow, the surface dismisses for the rest of the session — so power users who prefer typing aren't badgered.

A `FlowLayout: Layout` (Swift 5.9 `Layout` protocol implementation) wraps chip rows when the row width overflows. The implementation is the standard four-method shape: `sizeThatFits` walks subviews tracking row width and wrap height; `placeSubviews` walks again placing each at `(x, y)` with `topLeading` anchor.

**Refinement chips.** Once a `WorkoutPlan` arrives, a second chip surface appears between the messages list and the pin button, holding five canned-message buttons:

| Chip | Sends |
|---|---|
| Make it shorter / اقصرها | "Make it shorter — keep the essentials at the same quality." |
| Make it harder / صعّبها | "Push intensity up — add a set or harder variations." |
| Make it easier / سهّلها | "Tone it down to a beginner-friendly version of the same plan." |
| Swap exercises / بدائل | "Swap each exercise for an equivalent alternative." |
| No equipment / بدون معدّات | "Same plan but bodyweight only — no equipment." |

Each chip calls `globalBrain.sendMessage(...)` directly — the existing chat orchestration generates a fresh `WorkoutPlan` and the preview card replaces itself.

### 36.9 Exercise detail sheet — `ExerciseDetailSheet.swift`

[`AiQo/Features/Gym/Club/Plan/ExerciseDetailSheet.swift`](AiQo/Features/Gym/Club/Plan/ExerciseDetailSheet.swift) (285 LOC, new) is the bottom-sheet presented when the user taps any exercise — from the active plan card, the pending preview card, or the runner's up-next strip. Five stacked sections:

1. **Hero header** — 56pt gradient muscle-tinted circle, muscle label overline, exercise name, reps/duration line.
2. **Badge strip** — four square pills: `Sets` (count), `Approx` (`prettyDuration`), `Rest` (seconds), `Equip` (equipment label).
3. **Form cues** — bulleted list from `insights.formCues`, each row with a 7pt muscle-accented dot.
4. **Alternatives** — list from `insights.alternatives`, each row with a `arrow.triangle.swap` icon in a tinted circle.
5. **Pacing grid** — four mini-cards: `Total`, `Between sets`, `Muscle`, `Sets`.

Presentation uses `.presentationDetents([.medium, .large])` with `.presentationDragIndicator(.visible)` so users can grab and resize without hunting for a close button. The dismiss button (top-trailing X-in-circle on `.ultraThinMaterial`) sits inside an `.overlay(alignment: .topTrailing)` so it's reachable on either detent.

### 36.10 Dashboard + chat rewrite — `WorkoutPlanFlowViews.swift`

[`AiQo/Features/Gym/Club/Plan/WorkoutPlanFlowViews.swift`](AiQo/Features/Gym/Club/Plan/WorkoutPlanFlowViews.swift) is the only modified Swift file (1,276 LOC, +831/-274 vs the pre-upgrade 715 LOC). Both `WorkoutPlanDashboard` and `CaptainPlanChatView` are rewritten end-to-end while preserving their public API (the `MyPlanView.workoutPlanEntryCard` `NavigationLink` to `WorkoutPlanDashboard()` continues to compile unchanged, and `CaptainPlanChatView` is still presented from the dashboard's hero `NavigationLink`).

**Dashboard layout, top to bottom:**

1. **Hero** — `Captain Hamoudi` illustration + CTA. The copy switches based on whether a plan is pinned: "Your plan is ready 🎯 — open chat to refine or request a new plan" when active, the standard "Create a workout plan with the Captain" otherwise. Background gradient swaps mint→amber when active, mint→sky when empty.
2. **Weekly progress strip** — always visible.
3. **Active plan card** — only when `hasPinnedPlan && captainPlan != nil`. Carries the new "Start workout" button → presents the runner.
4. **Weekly stats hero** — only when `weeklyStats?.totalSessions > 0`.
5. **Quick-start templates strip** — always visible (gives a one-tap path even when a plan is already pinned).
6. **Health compliance card** (small).
7. **History list** — pinned plans from the last 20 days, each row showing date + completion ratio + first 3 exercises with checkboxes that mirror the SwiftData state.
8. **Empty-state feature card** — only when no plan is pinned and `globalBrain.currentWorkoutPlan == nil`. Four feature rows with icon+title+subtitle: smart personalization, clean exercises with sets/reps, progress tracking, Iraqi-dialect Captain.

The dashboard owns these state pieces that cross between the runner, the templates flow, and SwiftData:
...
The `pinTemplate(_:)` helper saves the selected template through `WorkoutPlanMemoryStore.savePlan(...)`, posts `.aiqoWorkoutPlanSaved` on `NotificationCenter`, sets `globalBrain.currentWorkoutPlan = plan` (so the runner finds it on first present), and reloads. On error it surfaces an Arabic/English alert.

The `markAllExercisesComplete()` callback wires `PlanWorkoutRunner.onCompleteAll` back to SwiftData — the runner's confetti screen marks every `WorkoutTask.isCompleted = true` so the dashboard's progress ring instantly fills to 100%, the streak counter increments, and the weekly stats update on next reload.

`pinnedPlanForToday()` is the round-trip read path — fetches today's `AiQoDailyRecord`, parses each `WorkoutTask.title` through `ExerciseSerialization.parse(taskTitle:)`, and reconstructs a `WorkoutPlan` whose title falls back to the locale-default ("خطة الكابتن" / "Captain's plan") if `record.captainDailySuggestion` is empty. The active plan card binds to this reconstructed plan, *not* to `globalBrain.currentWorkoutPlan` — so a user who pins, closes the app, and reopens it days later still sees the structured card, not legacy text.

**Chat layout, top to bottom:**

1. **Messages section** — preserved bubble rendering with the `.ultraThinMaterial` backdrop.
2. **Pending plan preview** (when `globalBrain.currentWorkoutPlan != nil`) — replaces the pre-upgrade flat list with the new `PendingPlanPreviewCard`.
3. **Intake chips** (when no user messages and no plan) — the four-step `PlanIntakeChipsView`.
4. **Refinement chips** (when a plan exists) — five canned-message buttons.
5. **Pin plan button** (when a plan exists) — gold→amber gradient capsule.
6. **Compliance + success/error cards.**
7. **Bottom-anchored input bar** — preserved free-form path.

`bootstrapPlanChatIfNeeded()` is unchanged in shape but now appends a friendlier kickoff line — see §36.11.

### 36.11 Files modified

**New files (7):**

- [`AiQo/Features/Gym/Club/Plan/WorkoutPlanInsights.swift`](AiQo/Features/Gym/Club/Plan/WorkoutPlanInsights.swift) — 498 LOC. The heuristic metadata layer. `ExerciseClassifier` lookup tables, `WorkoutMuscleGroup` / `WorkoutEquipment` / `WorkoutDifficulty` enums, `Exercise.insights(language:)` and `WorkoutPlan.insights(language:)` extensions.
- [`AiQo/Features/Gym/Club/Plan/ExerciseDetailSheet.swift`](AiQo/Features/Gym/Club/Plan/ExerciseDetailSheet.swift) — 285 LOC. The five-section bottom sheet.
- [`AiQo/Features/Gym/Club/Plan/WorkoutPlanCards.swift`](AiQo/Features/Gym/Club/Plan/WorkoutPlanCards.swift) — 718 LOC. `ActivePlanCard`, `ActivePlanExerciseRow` (private), `PendingPlanPreviewCard`, `WeeklyProgressStrip`, `DayProgress`.
- [`AiQo/Features/Gym/Club/Plan/WorkoutPlanIntakeChips.swift`](AiQo/Features/Gym/Club/Plan/WorkoutPlanIntakeChips.swift) — 377 LOC. The four intake enums, `PlanIntakeSelection`, `PlanIntakeChipsView`, `FlowLayout: Layout`.
- [`AiQo/Features/Gym/Club/Plan/PlanWorkoutRunner.swift`](AiQo/Features/Gym/Club/Plan/PlanWorkoutRunner.swift) — 784 LOC. The full-screen runner, `ConfettiView`, `ConfettiPiece`.
- [`AiQo/Features/Gym/Club/Plan/WorkoutTemplateLibrary.swift`](AiQo/Features/Gym/Club/Plan/WorkoutTemplateLibrary.swift) — 326 LOC. `WorkoutTemplate`, `WorkoutTemplateCatalog.all`, `QuickStartTemplatesStrip`.
- [`AiQo/Features/Gym/Club/Plan/PlanWeeklyStats.swift`](AiQo/Features/Gym/Club/Plan/PlanWeeklyStats.swift) — 197 LOC. `PlanWeeklyStats` value type, `WorkoutPlanMemoryStore.fetchWeeklyStats(...)` extension, `PlanWeeklyStatsHero`.

**Edited files (3):**

- [`AiQo/Features/Gym/Club/Plan/WorkoutPlanFlowViews.swift`](AiQo/Features/Gym/Club/Plan/WorkoutPlanFlowViews.swift) — 1,276 LOC (+831/-274 vs pre-upgrade). Full rewrite of `WorkoutPlanDashboard` and `CaptainPlanChatView`. New helpers: `pinnedPlanForToday()`, `completionByIndex()`, `toggleCompletion(at:)`, `markAllExercisesComplete()`, `pinTemplate(_:)`, `sharePlan()`, `pendingTemplateBinding`, `pinErrorBinding`. New persistence helpers: `WorkoutPlanMemoryStore.fetchTodayRecord(...)`, `.fetchWeeklyProgress(...)` (`DayProgress` shape), `.savePlan(...)` rewritten to use `ExerciseSerialization.encode(...)`. New types: `WorkoutPlanDailySnapshot` (now carries `suggestion: String?` and `completedCount: Int`), `WorkoutPlanDailyWorkout`, `ExerciseSerialization` enum, `WorkoutPlan.shareableText(language:)` extension.
- [`AiQo/Resources/ar.lproj/Localizable.strings`](AiQo/Resources/ar.lproj/Localizable.strings) — `gym.plan.kickoff` rewritten from `"حتى أبني خطة تمرين شخصية، اكتب وزنك الحالي وهدفك مثل: 95 كيلو وتنشيف."` to `"هلا بطل! اختار من الخيارات بالأسفل وأنا أبنيلك خطة تمرين شخصية بثواني، أو اكتب طلبك مباشرة."` to align with the chip flow that now sits right below it.
- [`AiQo/Resources/en.lproj/Localizable.strings`](AiQo/Resources/en.lproj/Localizable.strings) — same edit on the English side: `"Hey champ! Pick from the options below and I'll build a tailored plan in seconds — or just type your request directly."`.

All other UI strings (badge labels, button captions, empty-state copy, alert messages) are inlined directly in Swift source as bilingual ternaries `isArabic ? "..." : "..."` matching the §32 / §33 / §34 idiom — no new `gym.plan.*` keys were added. The trade-off is intentional: the 7 new files would have added ~80 keys to both `Localizable.strings` files for content that's contiguous with the surrounding view code; inline-bilingual keeps the file-level review cleaner and matches the §34 toast precedent.

Zero `pbxproj` edits. Zero changes to `WorkoutPlan` / `Exercise` decoder validation. Zero changes to `Brain/04_Inference/CaptainModels.swift`, `Brain/04_Inference/Services/HybridBrain.swift`, `Brain/04_Inference/Services/LocalBrain.swift`, or `PromptComposer.swift`. Zero new outbound HTTP calls. The §35 14-layer reasoning brain and the §32 / §34 app-knowledge prompt layers see no edits — the new helpers stay strictly UI-side.

### 36.12 Build & verification

`xcodebuild -project AiQo.xcodeproj -scheme AiQo -destination 'generic/platform=iOS Simulator' -configuration Debug build` returns `** BUILD SUCCEEDED **` with no new warnings on touched files. One compile error during development (`instance method 'autoconnect()' is not available due to missing import of defining module 'Combine'`) was resolved by adding `import Combine` to `PlanWorkoutRunner.swift`. The pre-existing SourceKit "Cannot find type X in scope" diagnostics on `Exercise` / `WorkoutPlan` / `AppLanguage` cleared once swiftc indexed — same root cause as §32.5, §33.9, §34.5.

The redundant `import UIKit` initially placed in `PlanWorkoutRunner.swift` for the haptic generators was dropped — `import SwiftUI` re-exports UIKit on iOS, matching the project convention used in `ProfileSetupView.swift`, `WaterDetailSheetView.swift`, and `HomeStatCard.swift`.

`AiQoTests` target compiles unchanged. No new tests landed with this pass — all the new types are pure-UI compositions or local heuristic helpers that read no global state. The `ExerciseClassifier` and `ExerciseSerialization` round-trip are the highest-value test targets and are explicitly named under §36.13 follow-ups.

### 36.13 Follow-ups

- **Round-trip persistence test.** `ExerciseSerialization.encode(...)` followed by `parse(taskTitle:)` should always yield the original `Exercise` for any well-formed input. A two-line `XCTAssertEqual` lock-in test in `AiQoTests` would catch any future change to the magic prefix or separator.
- **Legacy title fallback test.** `parseLegacyTitle(_:)` handles three Unicode dash variants (` - `, ` — `, ` – `) and digit-extracts the sets count; a fixture-based test asserts the parser doesn't regress when an old `WorkoutTask.title` shows up after a future migration.
- **Heuristic classifier coverage.** `ExerciseClassifier.muscleGroup(for:)` walks an Arabic-and-English keyword table that's currently informal — a test that runs a curated list of 40+ common exercise names in both languages and asserts the expected `WorkoutMuscleGroup` would catch regressions when new keywords are added.
- **Snapshot test for the runner completion overlay.** The confetti animation is decorative but the overlay's headline + stats line is content — a snapshot test on the static layout (animation paused) locks the layout against accidental regression.
- **Lift the curated template catalog to JSON.** Six templates today, but the catalog will grow. Moving `WorkoutTemplateCatalog.all` to a bundled JSON resource would let non-engineers add templates without touching Swift, and would let A/B tests swap catalogs without a binary release.
- **Wire the runner to `LiveWorkoutSession`.** The runner currently runs as a self-contained timer. Routing it through the existing `LiveWorkoutSession` (the Apple-Watch-aware path) would surface the workout in HealthKit, light up the Live Activity, and unify the two timers — at the cost of more state coupling. Defer until the watch surface is ready for plan-driven workouts.
- **Localize the inline bilingual strings.** The `isArabic ? "..." : "..."` ternaries are concise but they bypass the `gym.plan.*` key system used by the rest of the screen. A localization pass that lifts the ~80 inline strings to keys would also unblock future locale work (Levantine register, MSA register).
- **Telemetry on the runner.** Mirror §34.3 — a `CaptainMetricsCounter` event for `plan_runner_completed` (with `duration_seconds_total`, `sets_total`, `template_id?`) gives the dashboard the data it needs to validate that "world-class" is actually being used. Zero-PII contract carries forward.
- **Streak-break notification.** When `streakDays > 3` and the user hasn't pinned a plan yet today by 8pm local time, fire a `NotificationKind.streakRisk` through `NotificationBrain.shared.request(...)` — the §31 / §34 single-door pattern still applies. Out of scope for this pass; flagged here so the streak counter doesn't sit decorative.
- **Synthesize the §35 reasoning brief into the intake-chip composition.** Today the chips compose a flat user message that the brain then re-parses through `IntentClassifier` + `BehavioralStageEngine`. The chip selection is *itself* structured intent — wiring `PlanIntakeSelection` directly into the `ReasoningBrief` (skipping the natural-language round-trip) would save tokens and remove a parsing step. Cross-cuts §35.4.

---

## 37. Kitchen World-Class Development Pass (2026-05-10)

*Two-round build-out on branch `brain-refactor/p-fix-dev-override`, bundled into commit `85de575` alongside an unrelated gym-plan snapshot — Kitchen-only changes are listed in §37.10. Before this pass, Kitchen was a static meal-display: meals manually added through `QuickAddMealView` lived in a `@State` array on `DailyFoodLogView` and were lost on tab switch or app restart, planned-meal cards offered no way to record what the user actually ate, recipes had no cooking instructions, and the Kitchen had no awareness of trends, streaks, hydration, or fridge freshness. This pass turns Kitchen into a persistent daily-use coaching surface with a memory of its own — ten functional uplifts grouped into Round 1 (foundation) and Round 2 (world-class polish), all gated behind nothing but the existing AI-data consent and `TierGate` checks already in `KitchenPlanGenerationService`.*

### 37.1 Round 1 — Foundation: persistence, adherence, recipes, water, freshness

Six functional gaps closed in the first round, all on-device, no cloud dependency added.

**Persistent daily food log.** `DailyFoodLogView.manualMeals: [KitchenPlannedMeal]` was `@State` — a process-local cache. The fix introduces a new `LoggedMeal` value type that wraps a `KitchenPlannedMeal` with a `loggedAt: Date` and a `servings: Double` (used in Round 2), persists it through [`KitchenPersistenceStore.loggedMeals`](AiQo/Features/Kitchen/KitchenPersistenceStore.swift) (UserDefaults-backed, ISO-8601 dates, encoder/decoder configured at init), and exposes `logManualMeal(_:servings:loggedAt:)` + `removeLoggedMeal(id:)` + `loggedMealsToday(now:)` so the view layer never reaches for the raw array. The companion fix is a 30-day prune in `loadFromStorage()` so the store never grows unbounded — log + water + adherence rows older than 30 days are dropped on launch. The legacy hard-coded `dayIndex: 1` in `QuickAddMealView.saveMeal()` is left as a sentinel value because day-of-plan no longer matters for manual meals (planned-vs-logged is now distinguished by container, not by `dayIndex`).

**Per-meal+date adherence tracking.** A new `MealAdherenceState` enum (`pending` / `ate` / `skipped` / `swapped`) plus a `MealAdherenceRecord { state, loggedAt }` value type, indexed by a string key `"\(mealID.uuidString.lowercased())|yyyy-MM-dd"` (gregorian, en_US_POSIX) so each calendar day gets its own slate. The store exposes `adherenceState(for:on:)` and `setAdherence(_:for:on:)` (setting `.pending` deletes the row); the planned-today section in `DailyFoodLogView` renders three round buttons per meal — ✓ ate, ⤺ swapped, ✗ skipped — calling back through to `kitchenStore.setAdherence(...)`. The same chips appear on every today-row in `MealPlanView`, gated by a new `isMealForToday(_)` helper that derives the active day from the plan's `startDate`. `MealAdherenceState.countsTowardTotals` returns `true` for `.ate` and `.swapped` only — the `NutritionSummaryCard` reads from a unified list `kitchenStore.adheredPlannedMealsToday() + loggedToday.map(\.scaledMeal)` so calorie / protein / carb / fat totals reflect what was *actually* eaten, not what was *planned*.

**Recipe steps + cooking time.** `KitchenPlannedMeal` gains `steps: [String]` (default `[]`) and `cookingMinutes: Int?`. The structured-plan prompt in [`KitchenPlanGenerationService.buildStructuredPlanPrompt`](AiQo/Features/Kitchen/KitchenPlanGenerationService.swift) now requires Captain to emit "3-6 short numbered cooking steps per meal (each under 80 chars)" and a realistic `cookingMinutes` ≤ the user's preferred cooking time. The JSON schema example in the prompt is updated alongside, the `GeneratedMealPayload` Decodable struct gains the two new keys, and the parser cleans/filters the steps before mapping. The deterministic AR + EN fallback templates each meal type returns are extended end-to-end — both Arabic ("سخّن المقلاة على نار وسط" / "اخفق البيض") and English ("Heat the pan on medium" / "Whisk the eggs") variants now ship 3-5 steps plus cookingMinutes (10/25/8/1 for breakfast/lunch/dinner/snack). Backward compat is handled with an explicit `init(from:)` Decodable on `KitchenPlannedMeal` that defaults `steps` to `[]` and `cookingMinutes` to `nil` when missing — already-saved pinned plans deserialize cleanly without the new keys.

**Cook Mode.** [`CookModeView`](AiQo/Features/Kitchen/CookModeView.swift) — a new full-screen cover surfaced from `MealDetailSheet`'s "ابدأ الطبخ" / "Start cooking" CTA — turns a list of steps into a one-step-at-a-time walkthrough. Top header card shows the meal title + cookingMinutes + calories badges. Below it, a progress bar shows `completedSteps.count / totalSteps`. The current step card is large, step-numbered with a `Color.kitchenMint` circle, and the user can either tap the inline checkmark to mark this step done or hit the "التالي" / "Next" button which auto-marks-and-advances. When the last step is checked, a completion footer surfaces with a "بالعافية! 🎉" headline and (if the caller passed an `onMarkEaten` closure) a green "علّم وجبتك كمأكولة" button that writes adherence in one tap and dismisses. The closure is provided by `KitchenScreen` only when `pinnedMeal != nil`, so manually-logged meals (which already record adherence implicitly through being logged) don't get the prompt.

**Water tracking.** A new `WaterEntry { id, cups, loggedAt }` value type and three persistent helpers (`addWaterCup(now:)`, `removeLastWaterCupToday(now:)`, `waterCupsToday(now:)`). The `WaterTrackerCard` inside `DailyFoodLogView` shows current cups vs. an `@AppStorage("aiqo.nutrition.waterGoal")` default of 8, an animated mint progress bar, and a +/- pair of round buttons with `.light` haptic. Default goal is configurable via the existing `NutritionGoalsEditor` form. The water entry list contributes to the daily nutrition snapshot (`DailyNutrition.waterCups`) which Round 2's streak / insights consume.

**Fridge freshness.** `FridgeItem` gains `addedAt: Date` (defaulting to `updatedAt` when missing — handled in a new explicit `init(from:)`). Two thresholds (`staleSoonAfterDays: 5`, `staleAfterDays: 8`) feed a `freshness(now:)` enum returning `.fresh` / `.staleSoon` / `.stale` with corresponding emoji icons (`""` / `"⏳"` / `"⚠️"`). `FridgeDisplayItem` (the local presentation model in [`InteractiveFridgeView`](AiQo/Features/Kitchen/InteractiveFridgeView.swift)) carries the icon + day count; `FridgePinnedItemBadge` renders the freshness icon as a small white-capsule badge offset to the top-right of the emoji. An accessibility description reads "name, quantity, in fridge for X days" so VoiceOver users get the full context. The store-side filter `staleFridgeItems(now:)` excludes pantry-style essentials (carbs/fats/drinks/other categories from `IngredientCatalog`) so a half-used bag of rice doesn't trigger a stale warning — only fresh produce + dairy + protein + fruit categories surface as expiring.

**Today-remaining-calories chip.** A new chip top-right of the `KitchenScreen` header reads `\(remaining)\n\(localized("screen.kitchen.remaining"))` — flips to red text + red background tint when consumed exceeds goal, and the localized label flips from "سعرة متبقية" to "زيادة". The `caloriesConsumedToday()` helper sums `adheredPlannedMealsToday().compactMap(\.calories)` plus `loggedMealsToday().compactMap(\.meal.calories)` — same source-of-truth as `NutritionSummaryCard`.

### 37.2 Round 2 — World-class polish: streaks, weekly chart, smart insights, favorites, servings, macro budget, cooking timer

Six features layered on top of the foundation. Each one is computed from existing persistent state — no new storage axes beyond favorites — so the round is mostly view + helper compute, with the exception of `FavoriteMeal`.

**Streak system.** `KitchenPersistenceStore.currentStreak(calorieGoal:now:)` walks backward from today through `nutritionSnapshot(for:calorieGoal:)` per calendar day, incrementing while `met80PctGoal` (defined as `0.8 ≤ kcal/goal ≤ 1.15`) holds. Today is a *grace day*: if the user hasn't logged anything yet today, the cursor jumps straight to yesterday rather than breaking the streak — the original implementation broke the streak on an unlogged today, which is wrong because the user could have a 5-day run and just not have eaten breakfast yet. The streak is read by both the [`WeeklySummaryCard`](AiQo/Features/Kitchen/KitchenSmartCards.swift) (renders the 🔥 flame badge) and the `SmartInsightsCard` (uses it for the streak-celebration insight when `≥ 3`).

**7-day weekly chart.** `weeklySnapshot(calorieGoal:now:)` returns `[DailyNutrition]` for the last 7 calendar days (oldest first). Each entry carries `calories / protein / carbs / fat / waterCups / met80PctGoal`. The view renders a horizontal bar chart with each bar colored by status:

| Status | Range vs. goal | Color |
|---|---|---|
| Met goal | 80% ≤ kcal ≤ 115% | mint green `RGB(0.36, 0.74, 0.55)` |
| Under goal | kcal > 0 && kcal < 80% | sand gold `RGB(0.92, 0.78, 0.45)` |
| Over goal | kcal > 115% | terracotta `RGB(0.86, 0.42, 0.36)` |
| No data | kcal == 0 | secondary-tinted at 18% opacity |

A single-letter weekday label sits below each bar; a streak flame badge with the current run sits in the top-right of the card. The "حقّقت هدفك بـ N أيام" subtitle counts `goalDays = snapshot.filter(\.met80PctGoal).count` so the user sees their week-to-date adherence at a glance.

**Smart insights.** `computeInsights(inputs:now:)` is the most opinionated piece in this pass — seven generators ranked by importance, returning at most 3 results. Each insight is a `KitchenInsight { id, tone, icon, title, detail }` with one of four tones (`positive` / `attention` / `warning` / `info`) that drive the card's accent color in `SmartInsightsCard`.

| Insight ID | Trigger | Tone | Title example (AR) |
|---|---|---|---|
| `streak` | `streak ≥ 3` | positive | "🔥 سلسلة 4 أيام!" |
| `kcal-over` | `consumed > goal + 150` | warning | "تجاوزت هدف اليوم بـ 230 سعرة" |
| `kcal-low` | `hour ≥ 19 && consumed > 0 && consumed < 0.6 * goal` | attention | "بعد ناقصك 980 سعرة" |
| `protein-gap` | `proteinDeficit ≥ 30 && consumed > 200` | attention | "ناقصك 45 غرام بروتين" |
| `hydration` | `waterRemaining ≥ 4 && hour ≥ 12` | info | "بعد 5 أكواب ماء" |
| `skipped-<id>` | any planned meal today is `.skipped` | info | "تخطّيت الغداء اليوم" |
| `stale-fridge` | `staleFridgeItems().count ≥ 2` | attention | "3 عناصر بالثلاجة قاربت تنتهي" |
| `pending-meals` | only fires when `< 2` insights already, planned meals exist + some pending | info | "بعد 2 وجبات بالخطة" |

The `kcal-low` insight is intentionally late-day-only (hour ≥ 19) to avoid nagging at lunch about a late-evening goal. The protein-gap insight requires 200+ consumed calories so it doesn't fire at 7am after a single coffee. The `pending-meals` reminder is the lowest priority and only fires if fewer than 2 higher-priority insights already filled the slate. All copy is in dialect Iraqi/Gulf Arabic ("بعد ناقصك", "زيادة على هدفك") rather than literal MSA — matches Captain Hamoudi's tonal convention from §32.

**Meal favorites + recents.** A new `FavoriteMeal { id, meal, savedAt, lastUsedAt }` value type plus `favoriteMeals: [FavoriteMeal]` storage. Helpers:

- `toggleFavorite(_:)` — looks for an existing entry by `meal.id` OR by title+type similarity (`mealsAreSimilar`) so the same logical meal logged at different times stars/unstars consistently
- `isFavorite(_:)` — same equality semantics
- `recentlyUsedFavorites(limit:)` — sorts by `lastUsedAt` descending, returns up to N
- `recentlyLoggedMeals(limit:now:)` — independent of favorites; walks `loggedMeals` newest-first, deduplicates by lowercased title, returns up to N

`QuickAddMealView` gets a new `quickPickSection` at the very top of the form: two horizontal scrolls with rounded chip cards — favorites (mint-tinted, emphasized) and recents (tertiary-fill). Tapping any chip calls `onSave(meal, 1.0)` and dismisses immediately; favorites also call `kitchenStore.touchFavorite(id:)` so frequently-used meals stay at the front. A new "Save as favorite" toggle at the bottom of the form pairs with `kitchenStore.toggleFavorite(meal)` on submit. `MealPlanView` mirrors the favorites surface: every meal card now has a small star button in the top-right of its title row that toggles favorite state with a `.light` haptic.

**Servings adjuster.** `LoggedMeal.servings: Double` (default `1.0`, clamped to `≥ 0.25`) plus a computed `scaledMeal: KitchenPlannedMeal` that returns the meal with all macros multiplied by `servings`. `QuickAddMealView` gets a full-size pill stepper (-/+ at ±0.25, range 0.25–5.0) that displays `×1`, `×0.75`, `×1.5` etc. Each row in the daily-food-log list also carries a compact stepper that calls `kitchenStore.updateLoggedMealServings(id:servings:)` — instant macro recalc throughout (the `mealsCountedTowardTotals` computed property inside `DailyFoodLogView` picks up `\.scaledMeal` rather than `\.meal`). Backward compat: `LoggedMeal.init(from:)` defaults `servings` to `1.0` when the key is absent so older saved entries decode cleanly.

**Macro budget pill on plan cards.** Each card in `MealPlanView` shows a tiny pill next to the cooking-time pill: `\(percentage)% من اليوم` / `% of day`. The percentage is `meal.calories / globalCalorieGoal`. Color tier: green at < 30%, golden at 30-49%, orange at ≥ 50% — gives a glance-level read of whether one meal is going to dominate the day.

**Cooking timer in Cook Mode.** Two surfaces:

1. **Preset card** (visible while no timer running) — 4 quick chips (3 / 5 / 10 / 15 minutes). If the current step text contains a parseable minute hint (regex matches `\d{1,3}\s*(?:min|mins|minute|minutes|د|دقائق|دقيقة|دقيقتين|دقيقتان)`, gated to 1–90 min), a "مقترح: N د" sparkle chip appears in the top-right that starts a timer at the detected duration. The parser is exposed as `static func parseMinutes(from:) -> Int?` for unit-testability.
2. **Active timer card** (replaces the preset card while running) — shows monospaced `mm:ss` countdown with `.contentTransition(.numericText(countsDown: true))`, an animated mint progress bar, and a destructive "إلغاء" button. On `displayedRemaining == 0` and not yet fired: `UINotificationFeedbackGenerator(.success)` + `AudioServicesPlaySystemSound(SystemSoundID(1005))` (the standard iOS "Tweet sent" chime — chosen because it's short, distinctive, and doesn't sound like an alarm). The timer is driven by a `Timer.publish(every: 0.5, on: .main, in: .common).autoconnect()` and an `.onReceive(tickTimer)` recompute against an absolute `timerEndDate: Date?` so it's robust to view re-renders and accurate even if the half-second tick is delayed by event-loop pressure.

### 37.3 Models & types

All new types live in [`KitchenModels.swift`](AiQo/Features/Kitchen/KitchenModels.swift) — backward-compat decoders are added wherever a stored `Codable` type gains a new field.

| Type | Purpose | New fields | Decoder defaulting |
|---|---|---|---|
| `KitchenPlannedMeal` | Meal in a generated plan | `steps: [String]` (default `[]`), `cookingMinutes: Int?` | Yes — `decodeIfPresent` for both, `[]` and `nil` defaults |
| `FridgeItem` | Item in the smart fridge | `addedAt: Date` | Yes — `decodeIfPresent` falling back to `updatedAt` |
| `MealAdherenceState` | enum (`pending` / `ate` / `skipped` / `swapped`) | — | Pure enum, no migration |
| `MealAdherenceRecord` | `{ state, loggedAt }` | — | New type — keyed by `mealID|yyyy-MM-dd` |
| `LoggedMeal` | `{ id, meal, loggedAt, servings }` | `servings: Double` | Yes — defaults to `1.0` when absent |
| `WaterEntry` | `{ id, cups, loggedAt }` | — | New type, no migration |
| `FavoriteMeal` | `{ id, meal, savedAt, lastUsedAt }` | — | New type, no migration |
| `KitchenInsight` | `{ id, tone, icon, title, detail }` | — | Not persisted — pure value type for view layer |

`FridgeItem.FreshnessState` (`fresh` / `staleSoon` / `stale`) and the `freshness(now:staleSoonAfterDays:staleAfterDays:)` method are colocated.

### 37.4 Persistence layer expansion

[`KitchenPersistenceStore`](AiQo/Features/Kitchen/KitchenPersistenceStore.swift) gains four new `@Published` axes (`loggedMeals`, `adherence`, `waterEntries`, `favoriteMeals`) and ~482 LOC of new helpers. Every published axis follows the same pattern as the existing `fridgeItems` / `pinnedPlan` / `shoppingList`: `didSet` re-encodes through the shared `JSONEncoder` (ISO-8601 dates), `loadFromStorage()` reads through the shared `JSONDecoder`, and a single bootstrap flag `isBootstrapping` suppresses `didSet` writes during initial load.

| UserDefaults key | Type | Description |
|---|---|---|
| `aiqo.kitchen.log.meals` | `[LoggedMeal]` | Date-tagged manual food log (pruned > 30 days old) |
| `aiqo.kitchen.adherence` | `[String: MealAdherenceRecord]` | Per-meal+date state map (pruned > 30 days old) |
| `aiqo.kitchen.water.entries` | `[WaterEntry]` | Per-cup water log (pruned > 30 days old) |
| `aiqo.kitchen.favorites` | `[FavoriteMeal]` | User's saved favorite meals — never pruned |

The new compute helpers (all on the same store, all `@MainActor`):

| Method | Returns | Caller(s) |
|---|---|---|
| `logManualMeal(_:servings:loggedAt:)` | — | `DailyFoodLogView` add-meal sheet, favorite/recent quick-pick chips |
| `removeLoggedMeal(id:)` | — | `LoggedMealRow` trash button |
| `updateLoggedMealServings(id:servings:)` | — | `LoggedMealRow` compact stepper |
| `loggedMealsToday(now:)` | `[LoggedMeal]` | `DailyFoodLogView`, `KitchenScreen` calorie chip |
| `adherenceState(for:on:)` | `MealAdherenceState` | `DailyFoodLogView` planned section, `MealPlanView` adherence chips |
| `setAdherence(_:for:on:)` | — | Same callers, on tap |
| `adheredPlannedMealsToday(now:)` | `[KitchenPlannedMeal]` | `DailyFoodLogView` totals, `KitchenScreen` calorie chip |
| `addWaterCup(now:)` / `removeLastWaterCupToday(now:)` / `waterCupsToday(now:)` | — / — / Int | `WaterTrackerCard` |
| `toggleFavorite(_:)` / `isFavorite(_:)` / `touchFavorite(id:)` | — / Bool / — | `MealPlanView` star button, `QuickAddMealView` chips, "save as favorite" toggle |
| `recentlyUsedFavorites(limit:)` / `recentlyLoggedMeals(limit:now:)` | `[FavoriteMeal]` / `[KitchenPlannedMeal]` | `QuickAddMealView` quick-pick rows |
| `currentStreak(calorieGoal:now:)` | Int | `KitchenScreen` (passes to `WeeklySummaryCard`), `computeInsights` |
| `nutritionSnapshot(for:calorieGoal:)` | `DailyNutrition` | `currentStreak`, `weeklySnapshot`, `computeInsights` |
| `weeklySnapshot(calorieGoal:now:)` | `[DailyNutrition]` | `WeeklySummaryCard` |
| `computeInsights(inputs:now:)` | `[KitchenInsight]` | `SmartInsightsCard` |
| `staleFridgeItems(now:)` | `[FridgeItem]` | `SmartInsightsCard` (`stale-fridge` insight) |

Storage growth is bounded: `pruneStaleEntries()` runs once on init and drops anything older than 30 days from `loggedMeals` / `adherence` / `waterEntries`. Favorites are never pruned (low cardinality, user-curated).

### 37.5 Gemini structured-plan changes

[`KitchenPlanGenerationService.swift`](AiQo/Features/Kitchen/KitchenPlanGenerationService.swift) gets three coordinated edits:

1. **Prompt addendum** — two new bullets in the "Rules" block:
   ```
   ...
   ```
2. **Schema extension** — the example JSON in the prompt now includes `cookingMinutes: 20` on the meal object and a `steps: ["Heat the pan on medium", "Cook chicken 4 minutes per side", "Plate with rice and vegetables"]` array.
3. **Decoder extension** — `GeneratedMealPayload` gains `cookingMinutes: Int?` and `steps: [String]?`. The parser trims and filters empty strings before mapping to `KitchenPlannedMeal`.

The deterministic fallback (`deterministicFallbackPlan`) — invoked when the LLM round-trip fails or returns unparseable JSON — also propagates the new fields. `fallbackTemplate` returns a tuple now extended to `(title, calories, protein, ingredients, steps, cookingMinutes)`. Each of the eight branches (4 meal types × 2 languages) gets a hand-authored 3-5 step recipe and a realistic minute count: 10/25/8/1 for breakfast/lunch/dinner/snack. The Arabic copy reads naturally ("سخّن المقلاة على نار وسط" not literal MSA).

### 37.6 UI integration map

| Surface | Where | Change |
|---|---|---|
| `KitchenScreen` header | top of every Kitchen tab | New `todayCaloriesChip` showing kcal remaining (mint) or over (red) |
| `KitchenScreen` body | top of scroll | `SmartInsightsCard` (only if `insights.count > 0`) with spring transition |
| `KitchenScreen` body | between meal sections and buttons | `WeeklySummaryCard` (always visible) |
| `DailyFoodLogView` | inside scroll | `WaterTrackerCard` always present; `plannedTodaySection` + `loggedTodaySection` conditional |
| `MealDetailSheet` | added below ingredients | Cooking-time badge above stat cards, numbered steps section, "ابدأ الطبخ" button |
| `MealDetailSheet` | new full-screen cover | `CookModeView(meal:onMarkEaten:)` — only opened when `pinnedMeal?.steps` is non-empty |
| `MealPlanView` mealCard | top-right of title row | `favoriteToggle(meal:)` star button |
| `MealPlanView` mealCard | meta-info row | `cookingTimePill(minutes:)` and `macroBudgetPill(percentage:)` |
| `MealPlanView` mealCard | between meta-info and ingredients | `adherenceRow(meal:)` (today's meals only — gated by `isMealForToday(_)`) |
| `MealPlanView` mealCard | below ingredients | `stepsDisclosure(meal:)` — DisclosureGroup with numbered list |
| `QuickAddMealView` | top of form | `quickPickSection` — favorites + recents horizontal scrolls |
| `QuickAddMealView` | below macros grid | `servingsStepper` (×0.25 — ×5) and "احفظها كمفضّلة" toggle |
| `InteractiveFridgeView` | each `FridgePinnedItemBadge` | Freshness icon (⏳ / ⚠️) overlay on the emoji + accessibility-day-count |

The integration sequence inside `KitchenScreen.body`:

```
header (with todayCaloriesChip)
ScrollView {
    SmartInsightsCard         ← only if insights.count > 0
    DailyFoodLogView          ← summary card → water tracker → planned today → logged today → buttons
    breakfast section
    lunch section
    dinner section
    WeeklySummaryCard         ← always
    buttonsSection            ← regenerate / mealplan
}
```

The order is deliberate: insights first because they're the "what should I do right now" surface; daily log second because it's the primary daily affordance; the meal sections in the middle because they're the legacy surface (kept stable so users with existing flows aren't disoriented); the weekly summary near the bottom because it's a context layer rather than an action layer; and the regenerate/mealplan buttons last so they don't dominate the screen.

### 37.7 Localization

73 new keys across [`ar.lproj/Localizable.strings`](AiQo/Resources/ar.lproj/Localizable.strings) and [`en.lproj/Localizable.strings`](AiQo/Resources/en.lproj/Localizable.strings) (78-line additions per locale, identical key set, identical placeholders). Key prefixes:

| Prefix | Domain | Count |
|---|---|---|
| `kitchen.adherence.*` | The 4 adherence states + the prompt label | 5 |
| `kitchen.mealdetail.*` | Steps title, cookingMinutes formatter, "Start cooking" CTA | 3 |
| `kitchen.mealplan.*` | Steps disclosure label, cooking-minutes pill, favorite labels, budget pill | 5 |
| `kitchen.cook.*` | Cook Mode title, close, progress, allDone, stepOfTotal, markDone/Undone, navigation, completion, markEaten, noSteps | 12 |
| `kitchen.timer.*` | Title, suggested chip, running label, cancel, done message, a11y | 6 |
| `kitchen.weekly.*` | Title, subtitle, streak label, three legend chips | 6 |
| `kitchen.insight.*` | 7 insight title/detail pairs | 14 |
| `nutrition.*` (water + plannedToday + loggedToday + servings + favorites/recents + saveAsFavorite + adherence.progress) | Daily food log + servings + quick-pick row labels | 14 |
| `screen.kitchen.remaining` / `.over` / `.remaining.a11y` / `.over.a11y` | Header chip + accessibility | 4 |
| `fridge.freshness.daysInFridge` | Fridge badge accessibility | 1 |

The Arabic copy is dialect-aware ("بعد ناقصك", "خلّيك على الماء", "وقت تتعشّى") rather than MSA-formal, matching Captain Hamoudi's tonal convention from §32.

### 37.8 Files added / modified

**New files (2):**

- [`AiQo/Features/Kitchen/CookModeView.swift`](AiQo/Features/Kitchen/CookModeView.swift) — 583 LOC, the focused step-by-step cooking flow plus the integrated countdown timer + minute parser
- [`AiQo/Features/Kitchen/KitchenSmartCards.swift`](AiQo/Features/Kitchen/KitchenSmartCards.swift) — 263 LOC, `WeeklySummaryCard` + `SmartInsightsCard` + `SmartInsightRow`

**Modified files (9):**

- [`AiQo/Features/Kitchen/KitchenModels.swift`](AiQo/Features/Kitchen/KitchenModels.swift) — +227 LOC: new types + `FridgeItem.addedAt` + freshness + decoders
- [`AiQo/Features/Kitchen/KitchenPersistenceStore.swift`](AiQo/Features/Kitchen/KitchenPersistenceStore.swift) — +482 LOC: published axes + helpers + streak/snapshot/insights compute + 30-day prune
- [`AiQo/Features/Kitchen/KitchenPlanGenerationService.swift`](AiQo/Features/Kitchen/KitchenPlanGenerationService.swift) — +95 LOC: prompt + schema + parser + fallback templates extended
- [`AiQo/Features/Kitchen/NutritionTrackerView.swift`](AiQo/Features/Kitchen/NutritionTrackerView.swift) — +710 LOC: `DailyFoodLogView` rebuilt around the store, `WaterTrackerCard`, `PlannedMealAdherenceRow`, `LoggedMealRow` with compact stepper, `QuickAddMealView` with quick-pick rows + servings stepper + favorite toggle, `QuickPickItem` / `QuickPickChip`
- [`AiQo/Features/Kitchen/KitchenScreen.swift`](AiQo/Features/Kitchen/KitchenScreen.swift) — +232 LOC: header chip + insights+weekly card integration, `MealDetailSheet` with cookingTimeBadge / cookingStepsSection / startCookingButton + onMarkEaten plumbing
- [`AiQo/Features/Kitchen/MealPlanView.swift`](AiQo/Features/Kitchen/MealPlanView.swift) — +195 LOC: `favoriteToggle`, `cookingTimePill`, `macroBudgetPill`, `adherenceRow` + `adherenceChip`, `stepsDisclosure`, `isMealForToday`
- [`AiQo/Features/Kitchen/InteractiveFridgeView.swift`](AiQo/Features/Kitchen/InteractiveFridgeView.swift) — +36 LOC: `FridgeDisplayItem` carries freshness, `FridgePinnedItemBadge` renders the icon + a11y description
- [`AiQo/Resources/ar.lproj/Localizable.strings`](AiQo/Resources/ar.lproj/Localizable.strings) — +78 LOC: 73 new keys
- [`AiQo/Resources/en.lproj/Localizable.strings`](AiQo/Resources/en.lproj/Localizable.strings) — +78 LOC: same key set

Zero `pbxproj` edits — Xcode 16 `PBXFileSystemSynchronizedRootGroup` picks up the two new Swift files automatically.

### 37.9 Build & verification

`xcodebuild -project AiQo.xcodeproj -scheme AiQo -destination 'platform=iOS Simulator,name=Codex iPhone 16 Pro QA' -configuration Debug build` returns `** BUILD SUCCEEDED **`. The only warning emitted is a pre-existing one in [`PredictiveIntentEngine.swift:76`](AiQo/Features/Captain/Brain/03_Reasoning/PredictiveIntentEngine.swift) — `main actor-isolated static property 'unknown' can not be referenced from a nonisolated context` — unrelated to this pass (introduced by the §35 brain refactor work bundled into the same branch). No new warnings introduced by Kitchen work.

The pre-existing SourceKit "Cannot find type X in scope" diagnostics on `KitchenPersistenceStore` / `KitchenModels` / `MealPlanView` etc. are stale-cache artifacts of Xcode's indexer and resolve on the next clean rebuild — same root cause as §32.5, §33.9, §34.5, and §35. Actual `xcodebuild` is the source of truth and is green.

### 37.10 Commit & branch state

The pass landed in commit `85de575` on branch `brain-refactor/p-fix-dev-override`, pushed to origin the same day. The commit message is `chore: save in-progress kitchen + gym-plan work` — the Kitchen changes are bundled with an unrelated set of new gym-plan files (covered in §36) which were sitting in the working tree from a parallel session. Future archaeology should treat the Kitchen-only file list in §37.8 as the definitive scope of *this* development pass; the gym-plan files belong to §36, not the world-class kitchen brief.

### 37.11 Follow-ups

- **Photo capture per logged meal.** A `LoggedMeal.photoData: Data?` field plus a UIImagePickerController flow in `QuickAddMealView` would let users snap what they actually ate. Camera permission is already declared (§27); needs an entitlement-free PhotoKit save path or pure in-memory storage to stay clear of `NSPhotoLibraryAddUsageDescription` (which §27 explicitly stripped).
- **Apple Health write-back.** Logged meals currently live in UserDefaults only. Mirroring `caloriesConsumed` / `protein` / `carbs` / `fat` to `HKQuantityType.dietaryEnergyConsumed` etc. would surface AiQo nutrition in the Health app. Requires a new `NSHealthShareUsageDescription` purpose string and a write authorization step in Settings.
- **Weekly insights to Captain proxy.** `DailyNutrition` snapshots could be summarized into a single string passed to the cloud Brain via `HybridBrainRequest.workingMemorySummary` so Captain Hamoudi knows the user's recent eating pattern when answering generic questions. PrivacySanitizer already redacts the relevant numerics.
- **Streak protection / "rest day".** A user who consistently hits goal except on weekends shouldn't lose their entire streak — a 1-skip-per-7-day cushion would soften the cliff. Easy to add inside `currentStreak` with a small counter.
- **Notifications for skipped meals.** A new `NotificationKind.mealSkipped` routed through `NotificationBrain.shared.request(...)` could fire when `pendingToday.count == 1` at 14:00 / 19:00 thresholds. Cooldown + budget-gating already exist; only a new trigger registration needed.
- **Snapshot tests for `WeeklySummaryCard` + `SmartInsightsCard`.** Pure-render views with deterministic inputs — easy to lock down with `swift-snapshot-testing`. None added in this pass.
- **Unit tests for `currentStreak` / `computeInsights`.** Both are pure functions of `(plan, loggedMeals, adherence, waterEntries, calorieGoal, now)` — extracting them into a `KitchenInsightsEngine` value type would make them testable without instantiating a full `@MainActor` store. Worth doing before the surface gets more complex.
- **Lift Kitchen kcal envelopes to SSOT** — closes §34.6's deferred follow-up. The 380-450 / 500-600 / 400-500 ranges still live as TODO-marked literals in `appKnowledgeKitchen(_:)`. Surfacing them as `static let` on `MealPlanGenerator` makes them visible to both the prompt layer and Round 2's macro budget pill, which currently reads from `aiqo.nutrition.calorieGoal` only.
