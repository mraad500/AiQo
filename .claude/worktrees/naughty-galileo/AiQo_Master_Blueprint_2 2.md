...
| 4 | Profile Setup | `App/ProfileSetupView.swift` | Collects: name, age, height, weight, goals. Sets `didCompleteDatingProfile` |
...
8. **Legacy screen naming** — `ProfileSetupView.swift` now uses a function-based SwiftUI name, while `LoginViewController`, `GymViewController`, `LegacyCalculationViewController`, `RecapViewController`, `RewardsViewController`, `WinsViewController`, and `MyPlanViewController` still reflect legacy UIKit-era naming.
...
1. **UIKit ↔ SwiftUI bridge** — Several legacy UIKit-style screen names (`GymViewController`, `LoginViewController`, etc.) still coexist with SwiftUI views. `ProfileSetupView.swift` is already SwiftUI-native.
...
├── AuthFlowUI.swift                     # Sign in with Apple flow
├── ProfileSetupView.swift               # Profile setup (onboarding)
