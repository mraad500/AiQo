...
final class NotificationIntelligenceManager {
    ...
    func queueDeveloperTestSpiritualWhisper() async -> Bool {
        ...
        let coachLanguage = preferredCoachLanguage()
        let finalBody: String

        switch coachLanguage {
        case .english:
        ...
        let title = coachLanguage == .arabic ? "همسة كابتن حمودي" : "Captain Hamoudi"
    ...
    private func performInactivityCheckAndNotifyIfNeeded(
        ...
        let coachLanguage = preferredCoachLanguage()
        let composerLanguage: AppLanguage = coachLanguage == .english ? .english : .arabic
        ...
        let notificationTitle = coachLanguage == .arabic ? "همسة كابتن حمودي" : "Captain Hamoudi"
