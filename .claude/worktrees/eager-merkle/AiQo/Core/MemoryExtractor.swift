...
struct MemoryExtractor: Sendable {
    ...
    private static func extractWithRules(userMessage: String, store: MemoryStore) {
        ...
        // Mood / Emotional state (Arabic)
        let moodKeywordsAr: [(String, String)] = [
            ("متحمس", "متحمس"), ("تعبان", "تعبان"), ("مرهق", "مرهق"),
            ("قلقان", "قلقان"), ("مرتاح", "مرتاح"), ("محبط", "محبط"),
            ("زعلان", "زعلان"), ("طاقتي واطية", "طاقة منخفضة"), ("مو بمزاج", "مزاج سيء")
        ]
        for (keyword, moodValue) in moodKeywordsAr {
            if text.contains(keyword) {
                store.set("mood", value: moodValue, category: "mood", source: "extracted")
                break
            }
        }

        // Mood (English)
        let moodKeywordsEn: [(String, String)] = [
            ("stressed", "stressed"), ("anxious", "anxious"), ("tired", "tired"),
            ("exhausted", "exhausted"), ("motivated", "motivated"), ("happy", "happy"),
            ("low energy", "low energy"), ("frustrated", "frustrated"), ("relaxed", "relaxed")
        ]
        for (keyword, moodValue) in moodKeywordsEn {
            if text.localizedCaseInsensitiveContains(keyword) {
                store.set("mood", value: moodValue, category: "mood", source: "extracted")
                break
            }
        ...
        // Workout time preference (Arabic)
        let workoutTimeAr: [(String, String)] = [
            ("صباح", "morning"), ("فجر", "morning"), ("ظهر", "afternoon"),
            ("عصر", "afternoon"), ("مساء", "evening"), ("ليل", "night")
        ]
        for (keyword, timeValue) in workoutTimeAr {
            if text.contains(keyword) && (text.contains("تمرين") || text.contains("أتمرن") || text.contains("رياضة")) {
                store.set("preferred_workout_time", value: timeValue, category: "preference", source: "extracted")
                break
            }
        }

        // Workout time preference (English)
        if let match = text.range(of: #"(?:work ?out|train|exercise)\s*(?:in the\s*)?(morning|afternoon|evening|night)"#, options: [.regularExpression, .caseInsensitive]) {
            let timeMatch = String(text[match])
            let timeValue: String
            if timeMatch.localizedCaseInsensitiveContains("morning") { timeValue = "morning" }
            else if timeMatch.localizedCaseInsensitiveContains("afternoon") { timeValue = "afternoon" }
            else if timeMatch.localizedCaseInsensitiveContains("evening") { timeValue = "evening" }
    ...
    private static func extractWithLLM(
        ...
        let systemPrompt = """
        أنت محلل بيانات ذكي. من المحادثة التالية، استخرج أي معلومات جديدة عن المستخدم — سواء كانت أرقام صحية أو حالات جسدية/نفسية أو تفضيلات. أرجع JSON فقط بدون أي نص إضافي أو markdown.
        المفاتيح المسموحة فقط:
        * user_name, goal, weight, height, age
        * injury (أي ألم، إصابة، عملية سابقة، أو منطقة ضعيفة بالجسم)
        * mood (الحالة المزاجية أو العاطفية: متحمس، تعبان، قلقان، مرتاح، إلخ)
        * stress_level (مستوى التوتر إذا ذكره أو يُستنتج من كلامه)
        * energy_level (مستوى الطاقة: عالي، متوسط، منخفض)
        * preferred_workout, preferred_workout_time (الوقت المفضل للتمرين: صباح، ظهر، مساء)
        * diet_preference, sleep_hours, sleep_quality (جودة النوم: عميق، متقطع، خفيف)
        * fitness_level, workout_feedback, available_equipment
        * training_days, medical_condition, water_intake
        * pain_area (منطقة الألم المحددة إن ذُكرت: ركبة، ظهر سفلي، كتف أيمن، إلخ)
        * recovery_status (هل يحس بتعافي جيد أو تعب عضلي)
        * record_project_feedback
        إذا ما في شي جديد أرجع: {}
        مثال: {"weight": "95", "goal": "تنشيف", "mood": "متحمس", "preferred_workout_time": "صباح", "pain_area": "ركبة يسار"}
        """
        ...
        do {
            ...
            let categoryMap: [String: String] = [
                ...
                "mood": "mood", "stress_level": "mood", "energy_level": "mood",
                "preferred_workout": "preference", "preferred_workout_time": "preference",
                "diet_preference": "nutrition", "sleep_hours": "sleep",
                "sleep_quality": "sleep", "fitness_level": "body",
                "workout_feedback": "insight", "available_equipment": "preference",
                "training_days": "preference", "medical_condition": "injury",
                "water_intake": "nutrition", "pain_area": "injury",
                "recovery_status": "body",
