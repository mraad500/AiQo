...
struct CaptainDailyHealthMetrics: Sendable, Equatable {
    ...
    // Deep bio-state signals (nil = DATA_UNAVAILABLE)
    let hrvSDNN: Double?
    ...
    init(
        ...
        averageOrCurrentHeartRateBPM: Int?,
        sleepHours: Double,
        hrvSDNN: Double? = nil,
        ...
        self.averageOrCurrentHeartRateBPM = averageOrCurrentHeartRateBPM
...
final class CaptainIntelligenceManager {
    ...
    func fetchTodayEssentialMetrics() async throws -> CaptainDailyHealthMetrics {
        ...
        // Deep bio-state signals — gracefully nil if unavailable
        async let hrvValue = withHealthKitTimeout(fallback: nil as Double?) { [self] in
            try await fetchMostRecentDiscreteAverage(
                .heartRateVariabilitySDNN,
                unit: .secondUnit(with: .milli),
                interval: todayInterval
            )
        }
        async let restingHRValue = withHealthKitTimeout(fallback: nil as Double?) { [self] in
            try await fetchMostRecentDiscreteAverage(
                .restingHeartRate,
        ...
        async let waterValue = withHealthKitTimeout(fallback: nil as Double?) { [self] in
            let ml = try await fetchCumulativeQuantity(
                .dietaryWater,
        ...
        return await CaptainDailyHealthMetrics(
            ...
            hrvSDNN: hrvValue,
            restingHeartRate: restingHRValue.map { max(0, Int($0.rounded())) },
            waterIntakeML: waterValue
        )
    }
    ...
    private func buildContextPrompt(
        ...
        let heartRateText = metrics.averageOrCurrentHeartRateBPM.map { "\($0) bpm" } ?? "DATA_UNAVAILABLE"
        let sleepText = String(format: "%.1f", metrics.sleepHours)
        let hrvText = metrics.hrvSDNN.map { String(format: "%.0f ms", $0) } ?? "DATA_UNAVAILABLE"
        let restingHRText = metrics.restingHeartRate.map { "\($0) bpm" } ?? "DATA_UNAVAILABLE"
        let waterText = metrics.waterIntakeML.map { String(format: "%.0f mL", $0) } ?? "DATA_UNAVAILABLE"
        ...
        - HRV (SDNN): \(hrvText) [higher = better recovery/lower stress]
        - Resting Heart Rate: \(restingHRText) [lower = better cardiovascular fitness]
        - Water Intake: \(waterText)

        NOTE: "DATA_UNAVAILABLE" means the sensor has no reading yet today. Do NOT guess or fabricate values — acknowledge the gap naturally if relevant.
    ...
    /// Fetches the most recent discrete-average value for a given quantity type within the interval.
    /// Returns nil if no data is available (used for HRV, resting HR).
    private func fetchMostRecentDiscreteAverage(
        _ identifier: HKQuantityTypeIdentifier,
        unit: HKUnit,
        interval: DateInterval
    ) async throws -> Double? {
