...
- The raw discovery file count was `858`, but that count includes duplicate code under `.claude/worktrees/hopeful-feistel`.
- The live app-specific source/config inventory across `AiQo`, `AiQoWatch Watch App`, `AiQoWidget`, `AiQoWatchWidget`, and `Configuration` is `471` files.
- This blueprint is based on the code that exists today.
- When the requested filename or path did not exist, the document calls that out explicitly and points to the actual implementation file.
- When a conclusion is an inference from code organization rather than a literal string or constant, it is marked as `Inference`.

Requested-output filename conflict:

- The task title asked for `AiQo_Master_Blueprint_3.md`.
- Step 4 later said `AiQo_Master_Blueprint_2.md`.
- The confirmation string later said `AiQo_Master_Blueprint.3.md`.
- This file follows the top-level task name: `AiQo_Master_Blueprint_3.md`.
...
- `MainTabScreen` actually renders only:
  - home,
  - gym,
  - captain.
- No visible `tribe` tab is rendered in `TabView`.
- No visible `kitchen` tab is rendered in `TabView`.
- `kitchen` navigation is special-cased to:
  - switch to home,
...
- `Known issue:`
  - `MainTabRouter` declares five tab cases.
  - `MainTabScreen` renders three tab cases.
- Impact:
  - tab analytics can still log `tribe` and `kitchen`,
  - tab state can theoretically hold `.tribe`,
  - rendered tab UI does not expose matching items.
...
- Universal-link-style paths include tribe invite routing under:

```text
https://aiqo.app/tribe/join/<code>
```
...
### Singleton managers with persisted state

- `LevelStore.shared`
  - persists level and XP keys in `UserDefaults`.
- `StreakManager.shared`
  - persists streak keys and history.
- `FreeTrialManager.shared`
  - persists trial start date.
- `EntitlementStore.shared`
  - persists subscription id and expiration.
- `UserProfileStore.shared`
  - persists user profile and privacy mode.
- `MemoryStore.shared`
  - uses SwiftData rather than `UserDefaults`.
- `RecordProjectManager.shared`
  - uses SwiftData after migration.
- `ChallengeHistoryStore.shared`
  - persists completed arena challenges.
- `NotificationPreferencesStore`
  - persists notification-related preferences.

### UserDefaults key inventory highlights
...
- levels:
  ...
  - `aiqo.user.currentXP`
  - `aiqo.user.totalXP`
  - legacy migration keys also exist.
- streak:
  - `aiqo.streak.current`
  - `aiqo.streak.longest`
...
### SwiftData context usage patterns

- Dedicated container configuration happens up front at app launch.
- Some managers are configured with a `ModelContainer` and keep an internal `ModelContext`.
- SwiftData is used both for:
  - durable user state,
  - chat history,
  - challenge state,
  - arena entities,
  - long-term projects.
- `Inference:` the codebase prefers SwiftData for structured domain state and `UserDefaults` for lightweight settings / flags.

### Migration patterns

- `LevelStore` migrates legacy XP keys into new keys.
- `LegendaryChallengesViewModel` migrates old record-project state into SwiftData.
- The app contains legacy naming alongside modern keys, showing active migration rather than a clean-slate persistence scheme.
...
- Requested folder:
  - `AiQo/Features/Sleep/*.swift`
- File status:
  - File not found — sleep is not a standalone top-level feature module in the live source.
- Actual implementation lives across:
  - `AiQo/Features/Home/SleepDetailCardView.swift`
...
- Purpose:
  ...
  - recovery coaching,
  - “spiritual whispers” style notifications.
- AI routing:
  - local first for `sleepAnalysis`.
- Data models used:
  - Captain chat / memory state
...
- HealthKit types consumed:
  ...
  - heart rate
  - steps
  - calories as contextual recovery signals
- Feature flags:
  - no sleep-specific feature flag found.
- Current status:
  - `Inference: in-progress as a distributed capability`
  - reason: the functionality is real, but it is spread across services rather than owned by a dedicated feature module.
...
- HealthKit types consumed:
  - indirectly via points / level / challenge scores.
- Feature flags:
...
- Current status:
  ...
  - reason: the surface is large and visible, but backend repositories still proxy to mock data and store methods are stubbed.
...
- Purpose:
  ...
  - history,
  - leaderboards,
  - tribe-vs-tribe / global challenge framing.
- Key files:
...
- Feature flags:
  - inherited from tribe feature flags.
- Current status:
  - `Inference: in-progress`
  - reason: UI is advanced, but live backend parity is incomplete and some repositories remain mock-backed.
...
- `TRIBE_BACKEND_ENABLED` is `true` in `Info.plist`.
- `TribeRepositoryFactory` returns `SupabaseTribeRepository` when backend is enabled.
- But `SupabaseTribeRepository` currently just returns `MockTribeRepository` data.
- `SupabaseChallengeRepository` also delegates to `MockChallengeRepository`.
- `TribeStore.createTribe` and `joinTribe` still use explicit stub logic with TODO comments.
...
- `TRIBE_FEATURE_VISIBLE`
  - controls tribe visibility logic in tab / tribe experience flows.
- `TRIBE_SUBSCRIPTION_GATE_ENABLED`
  - controls whether tribe requires premium gating.

### Important reality gap

- Even with `TRIBE_BACKEND_ENABLED = true`, the “Supabase” repositories still delegate to mock repositories.
- So the flag signals intent, but not full backend completion.
...
- `AiQo.xcconfig` contains hard-coded Supabase URL/anon key defaults.
- `Secrets.xcconfig` contains live-looking Gemini, ElevenLabs, Spotify, and Supabase credentials.
- This is a release-risk and security-risk finding.

### How to enable / disable launch-sensitive features

- Tribe backend visibility/gating is controlled through the three Info.plist flags above.
- Language is controlled through onboarding and `LocalizationManager`.
- Notification-heavy behavior depends on:
  - onboarding completion,
  - notification permissions,
  - HealthKit permission flow state.
- Purchase / trial logic is governed by:
...
### Known issue 1 — Tab router and rendered tab UI diverge

- `AiQo/App/MainTabRouter.swift:9-15` declares five tabs:
  - home
  - gym
  - tribe
  - kitchen
  - captain
- `AiQo/App/MainTabScreen.swift:26-68` renders only three:
  - home
  - gym
  - captain
- `AiQo/App/MainTabRouter.swift:21-30` special-cases kitchen into a home notification rather than a tab.
- Risk:
  - navigation state and rendered UI are out of sync.

### Known issue 2 — Tribe backend flag says live, repositories still use mock data

- `AiQo/Info.plist` sets all tribe flags to `true`.
- `AiQo/Tribe/Repositories/TribeRepositories.swift:23-36` chooses “Supabase” repositories when backend is enabled.
- `AiQo/Tribe/Repositories/TribeRepositories.swift:285-299` shows those repositories delegating to mock repositories.
- Risk:
  - config suggests production readiness that backend code does not yet deliver.

### Known issue 3 — `TribeStore` still contains stub create/join logic

- `AiQo/Tribe/TribeStore.swift:63-79`
- `AiQo/Tribe/TribeStore.swift:99-119`
- The code explicitly says:
  - real Supabase flow not yet connected,
  - hidden via tribe feature visibility comment.
- Risk:
  - tribe creation/join experience is not production-safe as written.

### Known issue 4 — Secrets are checked into configuration files

- `Configuration/Secrets.xcconfig:4-16`
- `Configuration/AiQo.xcconfig:8-11`
- Risk:
  - service credential exposure,
  - accidental production misuse,
  - difficult rotation and environment separation.

### Known issue 5 — Filename typo in gym flow

- `AiQo/Features/Gym/WorkoutSessionScreen.swift.swift`
- Risk:
  - tooling confusion,
  - maintainability drag,
  - avoidable import/search friction.

### Known issue 6 — Sleep is important but not modularized

- No `AiQo/Features/Sleep` folder exists.
- Sleep is distributed across Home, Captain, HealthKit, and notifications.
- Risk:
  - ownership fragmentation,
  - harder future iteration,
  - mixed responsibility across files.

### Known issue 7 — Onboarding walkthrough file appears unwired

- `AiQo/Features/Onboarding/OnboardingWalkthroughView.swift` exists.
- `AppFlowController.RootScreen` does not route to it.
- Risk:
  - dead UI or unfinished onboarding path.

### UI / UX gaps identified

- No explicit onboarding paywall step, despite free-trial activation.
- Tribe visibility and tribe access logic do not line up with the main tab shell.
- Sleep capability lacks a single destination module.
- Settings live in `Core`, which is technically valid but makes feature ownership less obvious.
- Typography relies heavily on fixed sizes, which limits full Dynamic Type responsiveness.

### Missing features before TestFlight

- Finish real tribe repository integration.
- Remove checked-in secrets and replace with safe environment management.
- Resolve tab architecture mismatch.
- Verify crash-reporting backend linkage if Crashlytics is intended for release.
- Add clearer Dynamic Type and reduced-motion coverage across premium surfaces.
- Decide whether `OnboardingWalkthroughView` is active or should be removed.

### Missing features before App Store launch

- Production-grade tribe backend end-to-end.
- Security cleanup for xcconfig secrets.
- Final privacy review of all AI prompt payloads and logs.
- App Store Connect configuration validation:
  - category,
  - subscription metadata,
  - privacy nutrition labels.
- QA across iPhone + watch + widget ecosystems.

### Post-launch roadmap items inferred from code

- richer watch-to-phone coaching sync
- stronger tribe social loop
- clearer sleep/spirit destination module
- expanded legendary-project progression
- better Captain quality telemetry
- more robust backend/live data alignment across arena and tribe
