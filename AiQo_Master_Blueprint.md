...
- **Status (v1.0.7 build 30, shipping LIVE):** real shielding (ManagedSettings) + DeviceActivity scheduling + steps→unlock + escalation + Captain trainer all wired and build-green. Authorization is **`.individual`** (self-control, not the MDM-only `.child`). Protection can always be toggled off (with a "you'll break today's commitment" confirm), and deleting the app / revoking Screen Time releases every shield (OS guarantee). Zero IAP, zero cloud in the challenge loop, no blocked-app tokens ever leave the device. **Family Controls (Distribution)** entitlement **Apple-approved 2026-05-31** for the app id + 3 extension bundle ids ✓. Remaining before archive: on-device shielding test (simulator can't run it) + App Review access notes for the Max gate.
