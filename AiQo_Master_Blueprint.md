...
5. **Prompt composition** (7-layer system prompt — see §5.4).
...
### 5.9 Chat UI
`CaptainViewModel` (`@MainActor`) holds `messages`, `isLoading`, streaming state, `quickReplies`, `effectiveTier`. A `CoachCognitiveState` drives the "thinking" animation (`idle / readingMessage / thinkingOnDevice / shapingReply / typing`). Replies stream token-by-token (≈24-char chunks, ~16ms) for a typewriter effect; structured cards (plan, Spotify, quick replies) append after completion. History persists to SwiftData per session. Voice subsystem under `Captain/Voice/` (Apple + MiniMax providers, router, consent, cache, keychain). Scoped to its own conversation context (Plan has a separate Captain conversation).
