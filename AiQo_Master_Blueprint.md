...
- **Status (v1.0.7 build 30, shipping LIVE):** real shielding (ManagedSettings) + DeviceActivity scheduling + steps→unlock + escalation + Captain trainer all wired and build-green. Authorization is **`.individual`** (self-control, not the MDM-only `.child`). Protection can always be toggled off (with a "you'll break today's commitment" confirm), and deleting the app / revoking Screen Time releases every shield (OS guarantee). Zero IAP, zero cloud in the challenge loop, no blocked-app tokens ever leave the device. ⚠️ Submission prerequisite: **Family Controls (Distribution)** entitlement approved for the app id + 3 extension bundle ids; on-device shielding test recommended (simulator can't run it).
